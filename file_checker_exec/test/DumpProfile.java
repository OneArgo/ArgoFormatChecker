...
public class DumpProfile
{
      ...
      throws IOException
   {
      ...
      for (next = 0; next < args.length; next++) {
         if (args[next].equals("-help")) {
            Help();
            System.exit(0);
         } else if (args[next].equals("-only-first") ||
                    args[next].equals("-1")) {
            only_first = true;
         } else if (args[next].equals("-no-trailing-missing") ||
                    args[next].equals("-b")) {
            no_trailing = true;
         } else if (args[next].equals("-id-includes-prof") ||
                    args[next].equals("-i")) {
            id_inc_prof = true;
         } else if (args[next].equals("-group-prof") ||
                    args[next].equals("-g")) {
            group_prof = true;
      ...
      */
      ...
      for (String inFileName : inFileList) {
         ...
         stdout.printf(" %6d %8d %8d %10d %8d %9d  %s\n", 
                       nFile, nProf, nCalib, nHistory, nParam, nLevel, inFileName.trim());
               ...
               if (specVar.isParamVar()) {
                  ...
                  if (varName.equals(pName)) {
                     ...
                     if (r == 2) {
                        ...
                        if (gName != null) groupPrintVar.addAll(spec.groupMembers(gName));
                        if (altName != null) groupPrintVar.addAll(spec.groupMembers(altName));

                        log.debug("group-print variable '{}' (groups '{}', '{}'). rank = {}",
                                  pName, gName, altName, r);
         ...
         for (int n = 0; n < (only_first ? 1 : nProf); n++) {
            ...
            if (id_inc_prof) {
               outFile.printf("\n===> %s_%03d_%03d %10.3f %10.3f %s\n", 
                              plfm_num.trim(), cycle, n, lat, lon, juldDTG);
            } else {
            ...
            for (String varName : varNameList) {
               ...
               if (group_prof && 
                   groupPrintVar.contains(varName)) {
                  ...
                  log.debug(" variable isPerLevel: '{}'  rank: {}", 
                            varName, specVar.getRank());
                  ...
                  if (vRank == 2) {
                     ...
                     if (varType == DataType.CHAR) {
                        ...
                        /*
                          ...
                          int v[] = arFile.readIntArr(varName, n);

                          if (v != null) {
                          outFile.printf("   %-25s", varName);
                          for (int m = 0; m < v.length; m++) {
                          outFile.printf("   %5d) %15d\n", m, v[m]);
                          }
                          } else {
                          stdout.println("   "+varName+" not in the file");
                          }
                        */
                        ...
                        if (v != null) {
                           ...
                           if (len <= 0) {
                              outFile.printf
                              ("   %-25s No non-FillValue data for this profile\n", varName);
                        ...
                        stdout.println("   "+varName+": data type '"+varType+
                                       "' not handled");
                     }
                     ...
                     if (varType == DataType.CHAR) {
                        stdout.println("   "+varName+": not currently handling CHAR rank "+
                                    vRank);
                     } else if (varType == DataType.DOUBLE) {
                        stdout.println("   "+varName+": not currently handling DOUBLE rank "+
                                    vRank);
                     } else if (varType == DataType.FLOAT) {
                  ...
                  if (varType == DataType.CHAR) {
                     ...
                     stdout.println("   data type '"+varType+"' not handled for "+
                                    varName);
                  }

               } //..end if isPerProfile
               else {
                  log.debug(" independent variable: '{}'  rank: {}", 
                            varName, specVar.getRank());
   ...
   public static void Help ()
   {
      ...
      "\n"+
      "Usage: java DumpProfile [options] spec-dir "+
   ...
   public static void dumpGroup
      (ArgoProfileFile in, PrintWriter out, String groupName, DataType varType, int nProf,
       boolean no_trailing_missing)
   {
      ...
      float[] prm = null;      String sPrm = groupName;
      String  prm_qc  = null;  String sPQc = groupName+"_QC";
      float[] prm_adj = null;  String sAdj = groupName+"_ADJUSTED";
      float[] prm_err = null;  String sErr = groupName+"_ADJUSTED_ERROR";
      String  adj_qc  = null;  String sAQc = groupName+"_ADJUSTED_QC";
      ...
      if (prm == null) {
         stdout.println
         ("WARNING: STATION_PARAMETER 'groupName' is not a PARAM variable in the file ");
         return;
      }
