	...
	public static void main(String args[]) throws IOException {
		...
		for (next = 0; next < args.length; next++) {
			if (args[next].equals("-help")) {
				Help();
				System.exit(0);
			} else if (args[next].equals("-index-number") || args[next].equals("-i")) {
				ind_num = true;
			} else if (args[next].equals("-n_measure") || args[next].equals("-m")) {
				only_measure = true;
			} else if (args[next].equals("-n_cycle") || args[next].equals("-c")) {
				only_cycle = true;
		...
		stdout.println("\nProcessing File:");
		stdout.println(" FileNum  N_MEASUREMENT  N_CYCLE  N_PARAM  N_HISTORY  File Name");
		...
		for (String inFileName : inFileList) {
			...
			NetcdfFile ncFile = arFile.getNetcdfFile();
			...
			stdout.printf(" %6d %15d %8d %8d %10d  %s\n", nFile, nMeasure, nCycle, nParam, nHistory, inFileName.trim());
			...
			var = "FORMAT_VERSION";
			outFile.printf("   %-25s  %s\n", var, arFile.readString(var));
			var = "HANDBOOK_VERSION";
			outFile.printf("   %-25s  %s\n", var, arFile.readString(var));
			var = "REFERENCE_DATE_TIME";
			outFile.printf("   %-25s  %s\n", var, arFile.readString(var));
			var = "DATE_CREATION";
			outFile.printf("   %-25s  %s\n", var, arFile.readString(var));
			var = "DATE_UPDATE";
			...
			var = "WMO_INST_TYPE";
			outFile.printf("   %-25s  %s\n", var, arFile.readString(var));
			var = "POSITIONING_SYSTEM";
			outFile.printf("   %-25s  %s\n", var, arFile.readString(var));

			var = "TRAJECTORY_PARAMETERS";
			outFile.printf("   %-25s\n", var);
			...
			if (arFile.fileType() == ArgoDataFile.FileType.TRAJECTORY) {
			...
			outFile.printf(
					"   JULD variables  n)    MC   JULD/JULD_STATUS/JULD_QC | JULD_ADJUSTED/JULD_ADJUSTED_STATUS/JULD_ADJUSTED_QC\n");
			...
			if (arFile.fileType() == ArgoDataFile.FileType.TRAJECTORY) {
				...
				for (int n = 0; n < nMeasure; n++) {
					...
					outFile.printf("   %17d) %5d %15.6f  '%1s'  '%1s'  |  %15.6f  '%1s'  '%1s'\n", m, idata3[n],
							data1[n], data1_code.charAt(n), data1_qc.charAt(n), data2[n], data2_code.charAt(n),
							data2_qc.charAt(n));
				}
			} else {
				for (int n = 0; n < nMeasure; n++) {
					...
					outFile.printf("   %17d) %5d %15.6f  '%1s'  '%1s'  |  ---------------  '-'  '-'\n", m, idata3[n],
			...
			for (String pName : spec.getPhysicalParamNames()) {
				...
				if (prm_qc == null) { // ..PRES_QC is not in bio-traj files
					StringBuilder q = new StringBuilder(nMeasure);
					for (int n = 0; n < nMeasure; n++) {
						q.append('-');
					}
				...
				outFile.printf("   %-10s %11s / QC | %s_ADJUSTED / _ERROR / _QC\n", pName, pName, pName);
				...
				if (dprm != null) {
					if (dprm_adj == null) {
						...
						for (int n = 0; n < dprm.length; n++) {
							...
							outFile.printf("   %10d) %10.4f  '%1s' | %10.4f  %10.4f  '%1s'\n", m, dprm[n],
									prm_qc.charAt(n), dprm_adj[n], dprm_adj_err[n], prm_adj_qc.charAt(n));
						}
					}
					...
					if (fprm_adj == null) {
						...
						for (int n = 0; n < fprm.length; n++) {
							...
							outFile.printf("   %10d) %10.4f  '%1s' | %10.4f  %10.4f  '%1s'\n", m, fprm[n],
									prm_qc.charAt(n), fprm_adj[n], fprm_adj_err[n], prm_adj_qc.charAt(n));
						}
					}
					...
					if (sprm_adj == null) {
						...
						for (int n = 0; n < sprm.length; n++) {
							...
							outFile.printf("   %10d) %10d  '%1s' | %10d  %10d  '%1s'\n", m, sprm[n], prm_qc.charAt(n),
			...
			for (int n = 0; n < nCycle; n++) {
				...
				outFile.printf("   %10d) %6d  %6d  '%1s'  %6d  '%1s'\n", m, idata1[n], idata2[n], str1.charAt(n),
						idata3[n], str2.charAt(n));
			}

			for (ArgoReferenceTable.ArgoReferenceEntry e : ArgoReferenceTable.MEASUREMENT_CODE_toJuldVariable
					.values()) {
