...
public class DumpTraj32 {
	...
	public static void main(String args[]) throws IOException {
		...
		stdout.println("\nProcessing File:");
		stdout.println(
				" FileNum  N_MEASUREMENT  N_CYCLE  N_PARAM  N_HISTORY  " + "N_CALIB_JULD  N_CALIB_PARAM  File Name");
		...
		for (String inFileName : inFileList) {
			...
			stdout.printf(" %6d %15d %8d %8d %10d  %s\n", nFile, nMeasure, nCycle, nParam, nHistory, nCalibParam,
					nCalibJuld, inFileName.trim());
			...
			for (int n = 0; n < nMeasure; n++) {
				...
				outFile.printf("   %17d) %5d %15.6f  '%1s'  '%1s'  |  %15.6f  '%1s'  '%1s' | '%1s'\n", m, idata3[n],
						...
						data2_qc.charAt(n), data1_mode.charAt(n));
			}
					...
					if (dprm_adj == null) {
						for (int n = 0; n < dprm.length; n++) {
							...
							outFile.printf("   %10d) %10.4f  '%1s' | ----------  ----------  '-' | '%1s'\n", m, dprm[n],
									prm_qc.charAt(n), trajPrmDM[n].charAt(pNdx));
						}
					...
					if (sprm_adj == null) {
						...
						for (int n = 0; n < sprm.length; n++) {
							...
							outFile.printf("   %10d) %10d  '%1s' | %10d  %10d  '%1s' | '%1s'\n", m, sprm[n],
									prm_qc.charAt(n), sprm_adj[n], sprm_adj_err[n], prm_adj_qc.charAt(n),
									trajPrmDM[n].charAt(pNdx));
						}
					}
				}
