...
//................................................
	...
	protected NetcdfFile ncReader = null;
	protected String ncFileName = new String("");
	protected ArgoFileSpecification spec = null;
...
//.........................................
	...
	public static ArgoDataFile open(String inFile, String specDir, boolean fullSpec, String... dacName)
		...
		try {
			...
			if (e.getMessage().matches("cdlFileName.*does not exist")) {
				message = "File type / version not valid in the FileChecker: " + arFile.fileType + " / "
	...
	private boolean ckVarAttr(Variable dataVar, ArgoVariable specVar) {
		...
		for (Attribute dataAttr : dataVar.getAttributes()) {
			...
			// check if attribute is in specification
			if (specAttr == null) {
	...
	private void checkGroupsCompleteness(HashSet<String> dataElement, HashSet<String> dataGroup) {
		for (String group : dataGroup) {
			...
			if (vMiss.size() > 0) {
				...
				// ..add to formatErrors
				formatErrors.add("Parameter group " + group + ": Variables are missing for this group"
		...
		if (fileType == FileType.METADATA) {
			...
			if (fileType == FileType.PROFILE) {
				...
				if (data_mode == 'R' || data_mode == 'A') {
					...
					formatErrors.add("Could not determine file name: invalid DATA_MODE ='" + data_mode + "'");
					return null;
				}
			...
			if (direction == 'D') {
				...
				// ..do some rudimentary data checks here
				formatErrors.add("Could not determine file name: invalid DIRECTION ='" + direction + "'");
				return null;
			}
			...
			formatErrors.add("Could not determine file name: unexpected file type = '" + fileType + "'");
