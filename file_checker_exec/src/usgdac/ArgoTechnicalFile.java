 ...
 *
 * @version  $HeadURL: https://inversion.nrlmry.navy.mil/svn/godae/trunk/argo/bin/java/usgdac/ArgoTechnicalFile.java $
...
public class ArgoTechnicalFile extends ArgoDataFile
{
    ...
    *         successfully. 
    *         (ArgoTechnicalFile.getMessage() will return the reason for the 
    *         failure to open.)
    * @throws IOException If an I/O error occurs
    */
   public static ArgoTechnicalFile open (String inFile, String specDir, boolean fullSpec)
   throws IOException
   {
   ...
   /**
    * Validates the data in the technical file.
    ...
    * @throws IOException If an I/O error occurs
    */
   public boolean validate (String dacName, boolean ckNulls)
      throws IOException
   {
      ...
      if (creation.trim().length() <= 0) {
         ...
         if (dateCreation == (Date) null) {
            ...
            if (dateCreation.before(earliestDate)) {
               ...
               formatErrors.add("DATE_CREATION: '"+creation+
                  "': After system file time ('"+ArgoDate.format(fileTime)+"')");
            }
         }
      }
    ...
    * <i>obj</i>.formatErrors(), and <i>obj</i>.formatWarnings will return results.
    *
    * @param dac the ArgoReferenceTable.DACS dac indicator.  If <i>null</i> the DATA_CENTRE will not be checked
    * @throws IOException If an I/O error occurs
    */
   public void validateMetaData (ArgoReferenceTable.DACS dac)
   throws IOException
   {
      ...
      if (! s.matches("[1-9][0-9]{4}|[1-9]9[0-9]{5}")) {
         formatErrors.add("PLATFORM_NUMBER: '"+s+"': Invalid");
      }
      ...
      if (dac != (ArgoReferenceTable.DACS) null) {
         if (! ArgoReferenceTable.DacCenterCodes.get(dac).contains(s)) {
            formatErrors.add("DATA_CENTRE: '"+s+"': Invalid for DAC "+dac);
         }

      } else { //..incoming DAC not set
         if (! ArgoReferenceTable.DacCenterCodes.containsValue(s)) {
            formatErrors.add("DATA_CENTRE: '"+s+"': Invalid (for all DACs)");
         }
      }
    ...
    * @throws IOException If an I/O error occurs
    */
   public void validateTechParams()
      throws IOException
   {
      ...
      for (int n = 0; n < nParam; n++) {
         ...
         if (! nameAlreadyChecked.contains(param)) {
            ...
            ArgoConfigTechParam.ArgoConfigTechParamMatch match = 
               spec.ConfigTech.findTechParam(param);
          ...
          *
          * if (validUnit) {
          *    log.debug("valid unit");
          *
          *    if (! spec.ConfigTech.isConfigTechValidValue(unit, value[n].trim())) {
          *       String err = String.format("%s[%d]: Invalid value for '%s' (type '%s'): value = '%s'",
          *                                  vName, (n+1), full,
          *                                  spec.ConfigTech.getConfigTechDataType(unit), value[n].trim());
          *       //formatErrors.add(err);
          *
          *       //################# TEMPORARY WARNING ################
          *       formatWarnings.add(err + "   *** WILL BECOME AN ERROR ***");
