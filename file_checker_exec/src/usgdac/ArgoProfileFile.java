 ...
 * <p>
 * @version  $HeadURL: https://inversion.nrlmry.navy.mil/svn/godae/trunk/argo/bin/java/usgdac/ArgoProfileFile.java $
...
public class ArgoProfileFile extends ArgoDataFile
{
   ...
   /**
    * Creates a new Argo profile file. The "template" for the file is the
    * indicated CDL specification file.  The physical parameters (<PARAM> variables)
    * to be included must be specified -- see the "parameters" argument.
    * <p>
    * A few variables in the CDL spec can be float or double. 
    * These default to double.
    *
    ...
    * @param N_PARAM  N_PARAM dimension of the new file (max number of params in a single profile)
    ...
    */
   public static ArgoProfileFile createNew (String fileName, String specDir, 
                                            String version, 
                                            ArgoDataFile.FileType fileType,
                                            int N_PROF, int N_PARAM, int N_LEVELS,
                                            int N_CALIB, Set<String> inParameters)
      throws IOException, NumberFormatException
   {
      ...
      for (ArgoAttribute a : arFile.spec.getGlobalAttributes()) {
         ...
         if (name.equals("history")) {
            value = (new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'")).
      ...
      for (ArgoVariable v : orderedList) {
         ...
         if (keep) {
            ...
            if (v.canHaveAlternateDimensions()) {
               ...
               log.debug("modify alternate dims: before = '{}' after = '{}'",
                         dims, newDims);
               dims = newDims;
            }
    ...
    *         successfully. 
    *         (ArgoProfileFile.getMessage() will return the reason for the 
    *         failure to open.)
    * @throws IOException If an I/O error occurs
    */
   public static ArgoProfileFile open (String inFile)
      throws IOException
   {
      try {
   ...
   /**
    * Opens an existing file without opening the <i>specification</i> and, optionally,
    ...
    * @throws IOException If an I/O error occurs
    */
   public static ArgoProfileFile open (String inFile, boolean overrideBadTYPE) 
   throws IOException
   {
    ...
    * @throws IOException If an I/O error occurs
    */
   public static ArgoProfileFile open (String inFile, String specDir, boolean fullSpec)
   throws IOException
   {
   ...
   /**
    * Validates the data in the profile file.
    * This is a driver routine that performs all 
    * types of validations (see other validate* routines).  
    *
    * NOTE: This routine (and all other validate* routines) are designed
    * to handle both core-profile and bio-profile files.  There are 
    * some differnces in the validation checks that are noted where they occur.
    * 
    ...
    * @throws IOException If an I/O error occurs
    */
   public boolean validate (boolean singleCycle, String dacName, 
                            boolean ckNulls)
      throws IOException
   {
    ...
    * @throws IOException If an I/O error occurs
    */
   public void validateDates (int nProf, int nParam, int nCalib, int nHistory)
      throws IOException
   {
      ...
      for (int n = 0; n < nProf; n++) {
         ...
         if (qc_index >= 0) {
            ...
            if (haveCreation && (juldSec - creationSec) > oneDaySec) {
               formatErrors.add("JULD["+(n+1)+"]: "+juld[n]+" = '"+
                                juldDTG+"': After DATE_CREATION ('"+
                                creation+"')");
            }
            ...
            if (juld_loc[n] < 99990.d) {
               ...
               if (Math.abs(juld_loc[n] - juld[n]) > max) {
                  formatWarnings.add("JULD_LOCATION["+(n+1)+"]: "+juld_loc[n]+
                                     ": Not within "+max+" day of JULD ("+juld[n]+")");
               }
            ...
            for (int c = 0; c < nCalib; c++) {
               for (int p =0; p < nParam; p++) {
                  ...
                  if (dateCal.length() > 0) {
                     ...
                     if (date == (Date) null) {
                        formatErrors.add(calib_date+"["+(n+1)+","+(c+1)+","+
                              (p+1)+"]: '"+dateCal+"': Invalid date");
    ...
    * @throws IOException If an I/O error occurs
    */
   public void validateDMode (int nProf, int nParam, int nCalib, int nHist)
      throws IOException
   {
      ...
      ArrayChar cmts  = 
         (ArrayChar) findVariable("SCIENTIFIC_CALIB_COMMENT").read();
      //ArrayChar eqns  = 
      ...
      for (int n = 0; n < nProf; n++) {
         if (dMode.charAt(n) == 'D') {
            ...
            if (! (state.equals("2C") || state.equals("2C+"))) {
               formatErrors.add("D-mode: DATA_STATE_INDICATOR["+(n+1)+"]: '"+
                     state+"': Not set to \"2C\"");
            }
            for (int c = 0; c < nCalib; c++) {
               calibParam.clear();
               ...
               for (int p = 0; p < nParam; p++) {
                  ...
                  if (param.length() > 0) {
                     ...
                     if (! profQC.containsKey(pQcName)) {
                        ...
                        if (qc == null) {
                           formatErrors.add("D-mode: "+pQcName+" does not exist for"+
               ...
               for (String prm : profParam.get(n)) {
                  if (! calibParam.contains(prm)) {
                     formatErrors.add("D-mode: PARAMETER["+(n+1)+","+(c+1)+
            ...
            if (state.startsWith("2C")) {
               formatErrors.add("R/A-mode: DATA_STATE_INDICATOR["+(n+1)+"]: '"+
                     state+"': Can not be \"2C...\"");
            }
         }
      ...
      if (log.isDebugEnabled()){ log.debug("nHist: "+nHist); }
      if (nHist < 1) {
    ...
    *
    * Highly Desirable Checks:
    *    INST_REFERENCE: Set
    *    POSITIONING_SYSTEM: Set and valid (reference table 9)
    * 
    * <p>Upon completion <i>obj</i>.nFormatErrors(), <i>obj</i>.nFormatWarnings, 
    * <i>obj</i>.formatErrors(), and <i>obj</i>.formatWarnings will return results.
    *
   ...
   public void validateHighlyDesirable (int nProf) throws IOException
   {
      ...
      if (spec.getVariable("INST_REFERENCE") != (ArgoVariable) null) {
      ...
      if (spec.getVariable("POSITIONING_SYSTEM") != (ArgoVariable) null) {
         ...
         for (int n = 0; n < nProf; n++) {
            ...
            ArgoReferenceTable.ArgoReferenceEntry info = 
               ArgoReferenceTable.POSITIONING_SYSTEM.contains(str[n].trim());
            if (! info.isActive) {
               formatWarnings.add("POSITIONING_SYSTEM["+(n+1)+"]: '"+str[n]+
                                     "' Status: "+info.message);
            }
         }
      }
    ...
    *
    * @param nProf the number of profiles in the file
    * @param dac the ArgoReferenceTable.DACS dac indicator.  If <i>null</i> the DATA_CENTRE cannont be
    * @param singleCycle true = perform single-cycle test; false = accept multi-cycle files
    * validated.
    * @throws IOException If an I/O error occurs
    */
   public boolean validateMetaData (int nProf, ArgoReferenceTable.DACS dac, boolean singleCycle)
   throws IOException
   {
      ...
      for (int n = 0; n < nProf; n++) {
         ...
         if (s.length() == 0) {
            ...
            formatErrors.add("DATA_STATE_INDICATOR["+(n+1)+"]: '"+s+
                             "' Invalid");
         }
         ...
         if (dac != (ArgoReferenceTable.DACS) null) {
            if (! ArgoReferenceTable.DacCenterCodes.get(dac).contains(dc[n].trim())) {
               formatErrors.add("DATA_CENTRE["+(n+1)+"]: '"+dc[n]+
                     "': Invalid for DAC '"+dac+"'");
            }
            ...
            try {
               ...
               if ( (info = ArgoReferenceTable.WMO_INST_TYPE.contains(N)).isValid() ) {
                  if (info.isDeprecated) {
                     formatWarnings.add("WMO_INST_TYPE["+(n+1)+"]: '"+s+
                                        "' Status: "+info.message);
                  }
               } else {
                  formatErrors.add("WMO_INST_TYPE["+(n+1)+"]: '"+s+
                                   "' Status: "+info.message);
               }
         ...
         if (s.length() == 0) {
            ...
            if (hasData == null) {
               ...
               hasData = has_data;
               log.debug("...vertical_sampling_scheme empty. searched PRES. has_data = {}",
                         has_data);

            } else {
               has_data = hasData;
               log.debug("...vertical_sampling_scheme empty. PRES already searched. has_data = {}",
                         has_data);
            }
            ...
            if ( (info = ArgoReferenceTable.VERTICAL_SAMPLING_SCHEME.contains(s)).isValid() ) {
               ...
               String err = String.format("VERTICAL_SAMPLING_SCHEME[%d]: Invalid: '%s'",
                                          (n+1), s.trim());
               //formatErrors.add(err);
    ...
    * @throws IOException If an I/O error occurs
    */
   public void validateParams(int nProf, int nParam, int nLevel) 
      throws IOException
   {
      ...
      for (int profNum = 0; profNum < nProf; profNum++) {
         ...
         if (mode != 'A'  &&  mode != 'D'  &&  mode != 'R') {
            formatErrors.add("DATA_MODE["+(profNum+1)+"]: '"+
                  mode+"': Invalid");
         }
         ...
         if (param_mode != null) {
            ...
            for (int paramNum = 0; paramNum < nParam; paramNum++) {
               ...
               if (md != 'A'  &&  md != 'D'  &&  md != 'R'  &&  md != ' ') {
                  formatErrors.add("PARAMETER_DATA_MODE["+(profNum+1)+","+(paramNum+1)+
                                   "]: '"+md+"': Invalid");
               }
               ...
               if (fileType == FileType.BIO_PROFILE &&
                   stParam[paramNum].trim().startsWith("PRES")) {
                  if (md != 'R') {
                     log.debug("PRES[{}]: PARAMETER_DATA_MODE[{},{}] = '{}'. must be 'R'", 
                               profNum, profNum, paramNum, md);
            ...
            if (final_mode == ' ') {
               ...
               if (mode != 'R') {
                  ...
                  log.debug("data_mode[{}] != 'R'. All param_data_mode[{},...] are ' '", 
                            profNum, profNum);
               }
               ...
               log.debug("data_mode[{}]/param_data_mode[{},{}] inconsistent", 
                         profNum, profNum, final_nparam);
            }
            ...
            if (param.length() == 0) {
               ...
               if (allowedParam.contains(param)) {
                  ...
                  if (profParam.get(profNum).contains(param)) {
                     ...
                     formatErrors.add("STATION_PARAMETERS["+(profNum+1)+","+
                           (paramNum+1)+"]: '"+param+"': Duplicate entry");
                  ...
                  if (spec.isDeprecatedPhysicalParam(param)) {
                     ...
                     formatWarnings.add("STATION_PARAMETERS["+(profNum+1)+","+
                           (paramNum+1)+"]: '"+param+"': Deprecated parameter name");
                  }

               } else {
                  //..<param> is illegal
                  formatErrors.add("STATION_PARAMETERS["+(profNum+1)+","+
                        (paramNum+1)+"]: '"+param+"': Invalid parameter name in this context");
               }
         ...
         if (log.isDebugEnabled()){log.debug("paramList["+profNum+"]: "+paramList);}

         if (nParamUsed > maxParamUsed) maxParamUsed = nParamUsed;

         //..report errors and warnings
         if (embeddedEmpty) {
            formatWarnings.add("STATION_PARAMETERS["+(profNum+1)+
                  ",*]: Empty entries in list"+"\n\tList: "+paramList);
         }
         ...
         if (profNum == 0) {
            for (String p : allowedParam) {
               if (! spec.isOptional(p)  &&  ! profParam.get(profNum).contains(p)) {
                  formatErrors.add("STATION_PARAMETERS["+(profNum+1)+
                                   ",*]: Required PARAM ('"+p+"') not specified");
                  fatalError = true;
               }
         ...
         for (String p : profParam.get(profNum)) {
            ...
            if (var == (Variable) null) {
               formatErrors.add("STATION_PARAMETERS["+(profNum+1)+
                     ",*]: PARAM '"+p+"' specified. Variables not in data file.");
               fatalError = true;
            }
         }//..end profParam
            ...
            if (! profParam.get(profNum).contains(p)) {
               ...
               if (var != (Variable) null) {
                  ...
                  if (type == DataType.DOUBLE) {
                     ...
                     throw new IOException("Invalid data type for '"+
                                           p+"' (validateParams)");
                  }
                  
                  if (hasData) {
                     formatErrors.add("STATION_PARAMETERS["+(profNum+1)+
                           ",*]: Does not specify '"+p+"'. Variable contains data.");
         ...
         PARAM_LOOP:
         for (String param : profParam.get(profNum)) {
            ...
            //..what is needed: level-by-level is there any Nan, any not-fill, or is it all fill
            ...
            if (vRank == 2) {
               ...
               for (int k = 0; k < nLevel; k++) {
                  ...
                  if (is_nan) { prm[k] = Float.NaN; } //..NaN trumps everything
                  else if (data != Float.MAX_VALUE) { prm[k] = data; } //..had data
                  else { prm[k] = fValue; }
               }
               //log.debug("(temp) prm: {}", Arrays.toString(prm));
            }
            ...
            try {
               ...
               stderr.println(e.getMessage());
               e.printStackTrace(stderr);
               throw new IOException ("Unable to read "+varName+
                     "_QC["+profNum+"]: InvalidRangeException");
            ...
            for (int k = 0; k < prm.length; k++) {
               //if (! ArgoDataFile.is_99_999_FillValue(prm[k])  &&  k > maxLevelUsed) maxLevelUsed = k;
               if (prm[k] == Float.NaN) nan++;
               ...
               if ( (info = ArgoReferenceTable.QC_FLAG.contains(prm_qc[k])).isValid() ) {
                  ...
                  if (ArgoDataFile.is_FillValue(fValue, prm[k])) {
                     ...
                     if (prm_qc[k] != '9' &&  prm_qc[k] != '0') { notMiss++; }
                  ...
                  if (prm_qc[k]=='1' || prm_qc[k]=='2' || 
                      prm_qc[k]=='5' || prm_qc[k]=='8') {
                     n_good++;
                  } else if (prm_qc[k] == '0') {
                     n_noqc++;
                  }
            ...
            if (log.isDebugEnabled()) {
               ...
               log.debug("checking {}[{}]: n_data, n_good, n_noqc, = {}, {}, {}",
                         varName, profNum, n_data, n_good, n_noqc);
            }

            //..report errors and warnings
            if (invQC > 0) {
               paramErr = true;
               formatErrors.add(varName+"_QC["+(profNum+1)+
                     "]: Invalid QC codes at "+invQC+" levels (of "+prm.length+")");
            }
            if (illQC > 0) {
               paramErr = true;
               formatErrors.add(varName+"_QC["+(profNum+1)+
                     "]: QC codes not '1' to '4' at "+illQC+" levels with data (of "+
                     prm.length+")");
            }
            if (depQC > 0) {
               formatWarnings.add(varName+"_QC["+(profNum+1)+
                     "]: Deprecated QC codes at "+depQC+" levels (of "+
                     prm.length+")");
            }
            if (noQC > 0) {
               paramErr = true;
               formatErrors.add(varName+"_QC["+(profNum+1)+
                     "]: QC code '0' at "+noQC+" levels (of "+prm.length+")");
            }
            if (notMiss > 0) {
               paramErr = true;
               formatErrors.add(varName+"_QC["+(profNum+1)+
                     "]: Missing data but QC not missing at "+
                     notMiss+" levels (of "+prm.length+")");
            }
            if (notNotMeas > 0) {
               paramErr = true;
               formatErrors.add(varName+"_QC["+(profNum+1)+
                     "]: Blank (' ') QC when data is not missing at "+
            ...
            if (nan > 0) {
               formatErrors.add(varName+"["+(profNum+1)+
                     "]: NaNs at "+nan+" levels (of "+prm.length+")");
            }
            ...
            if (! paramErr) {
               ...
               try {
                  ...
                  stderr.println(e.getMessage());
                  e.printStackTrace(stderr);
                  throw new IOException ("Unable to read "+varName+"["+profNum+
                                         "]  (validateParams)");
               }
               ...
               if (vRank == 2) {
                  ...
                  for (int k = 0; k < nLevel; k++) {
                     ...
                     if (is_nan) { prm_adj[k] = Float.NaN; } //..NaN trumps everything
                     else if (data != Float.MAX_VALUE) { prm_adj[k] = data; } //..had data
                     else { prm_adj[k] = fValue; }
                  }
               ...
               if (mode == 'R') {
                  ...
                  for (int k = 0; k < prm_adj.length; k++) {
                     if (! ArgoDataFile.is_FillValue(fValue, prm_adj[k])) missNot++;
                     if (! ArgoDataFile.is_FillValue(fValue, prm_adj_err[k])) errNotMiss++;
                     if (prm_adj_qc[k] != ' ' && 
                         prm_adj_qc[k] != '0' && 
                         prm_adj_qc[k] != '9') {
                        qcNotMiss++;
                  ...
                  if (missNot > 0) {
                     paramErr = true;
                     formatErrors.add("DATA_MODE '"+mode+"': "+varName+"["+
                           (profNum+1)+"]: Not missing at "+
                           missNot+" levels (of "+prm.length+")");
                  }
                  if (errNotMiss > 0) {
                     paramErr = true;
                     formatErrors.add("DATA_MODE '"+mode+"': "+varName+"_ERROR["+
                           (profNum+1)+"]: Not missing at "+
                           errNotMiss+" levels (of "+prm.length+")");
                  }
                  if (qcNotMiss > 0) {
                     paramErr = true;
                     formatErrors.add("DATA_MODE '"+mode+"': "+varName+"_QC["+
                           (profNum+1)+"]: Not missing at "+
               ...
               //..... do checks for A and D files ........
                  ...
                  /* 2021-02: A-mode files can now have _ADJUSTED_ERROR set
                  int errNotMissAmode = 0; //..count of ERROR not missing in A-mode file
                  */

                  for (int k = 0; k < prm_adj.length; k++) {
                     if (prm_adj[k] == Float.NaN) nan++;
                     if (prm_adj_err[k] == Float.NaN) nanErr++;
                        ...
                        if (ArgoDataFile.is_FillValue(fValue, prm[k])) {
                           ...
                           if (ArgoDataFile.is_FillValue(fValue, prm_adj[k])) {
                              ...
                              if (prm_adj_qc[k] == '4' || prm_adj_qc[k] == '9') {
                                 ...
                                 /* 2021-02: A-mode files can now have _ADJUSTED_ERROR set
                                  *          Remove this test
                                  *          --- per e-mails from JP Rannou and Annie
                                 //} else {
                                 */
                                 }
                              }
                     ...
                     if (prm_adj_qc[k]=='1' || prm_adj_qc[k]=='2' || 
                         prm_adj_qc[k]=='5' || prm_adj_qc[k]=='8') {
                        n_good++;
                     ...
                     if (is_FillValue(fValue, prm_adj[k]) ^ 
                         is_FillValue(fValue, prm_adj_err[k])) {
                        mismatchAdjErr++;
                     }
                  ...
                  if (log.isDebugEnabled()) {
                     log.debug("checking {}[{}]: invQC, missAdj, missNot, missPrm, "+
                          "qcNotMiss, notNotMeas, depQC = {}, {}, {}, {}, {}, {}, {}",
                          varName, profNum,
                          invQC, missAdj, missNot, missPrm, qcNotMiss, notNotMeas, depQC);
                     log.debug("checking {}[{}]: errNotMiss, errMiss, "+
                               "nan, nanErr = {}, {}, {}, {}",
                               varName, profNum, 
                               errNotMiss, errMiss, nan, nanErr);
                  ...
                  if (invQC > 0) {
                     param_adjErr = true;
                     formatErrors.add(varName+"_QC["+(profNum+1)+
                           "]: Invalid QC codes at "+invQC+
                           " levels (of "+prm_adj.length+")");
                  }
                  if (depQC > 0) {
                     formatWarnings.add(varName+"_QC["+(profNum+1)+
                           "]: Deprecated QC codes at "+depQC+
                           " levels (of "+prm_adj.length+")");
                  }
                  if (incNotMeas > 0) {
                     param_adjErr = true;
                     formatErrors.add("DATA_MODE '"+mode+"': "+
                           varName+"_QC["+(profNum+1)+
                           "]: Incompatible blank (' ') QC codes at "+
                           incNotMeas+" levels (of "+prm_adj.length+")");
                  }
                  if (notNotMeas > 0) {
                     param_adjErr = true;
                     formatErrors.add(varName+"_QC["+(profNum+1)+
                           "]: Blank (' ') when data not missing at "+
                  ...
                  if (missAdj > 0) {
                     param_adjErr = true;
                     formatErrors.add("DATA_MODE '"+mode+"': "+param+"["+(profNum+1)+
                           "] Not missing / "+varName+" Missing (QC not 4 or 9): At "+
                           +missAdj+
                           " levels (of "+prm_adj.length+")");
                  }
                  if (missMiss > 0) {
                     param_adjErr = true;
                     formatErrors.add(varName+"_QC["+(profNum+1)+
                           "] Missing / "+param+"_QC Not missing: At "+missMiss+
                           " levels (of "+prm_adj.length+")");
                  }
                  if (missNot > 0) {
                     param_adjErr = true;
                     formatErrors.add("DATA_MODE '"+mode+"': "+varName+"["+(profNum+1)+
                           "]: Not missing when QC = 4 or 9 at "+missNot+
                           " levels (of "+prm_adj.length+")");
                  }
                  if (missPrm > 0) {
                     param_adjErr = true;
                     formatErrors.add("DATA_MODE '"+mode+"': "+param+"["+(profNum+1)+
                           "] Missing / "+varName+" Not missing: At "+missPrm+
                           " levels (of "+prm_adj.length+")");
                  }
                  if (qcNotMiss > 0) {
                     param_adjErr = true;
                     formatErrors.add(varName+"_QC["+(profNum+1)+
                           "]: Missing data but QC not missing at "+qcNotMiss+
                           " levels (of "+prm_adj.length+")");
                  }
                  if (errNotMiss > 0) {
                     param_adjErr = true;
                     formatErrors.add(varName+"_ERROR["+(profNum+1)+
                           "]: Not missing when PARAM or _ADJUSTED is missing at "+
                           errNotMiss+
                           " levels (of "+prm_adj.length+")");
                  }
                  if (errMiss > 0) {
                     param_adjErr = true;
                     formatErrors.add("DATA_MODE: '"+mode+"': "+varName+"_ERROR["+
                           (profNum+1)+"]:  Incorrectly set to missing at "+errMiss+
                           " levels (of "+prm_adj.length+")");
                  }
                  if (mode == 'D' && mismatchAdjErr > 0) {
                     ...
                     formatErrors.add("DATA_MODE: '"+mode+"': "+varName+
                           "["+(profNum+1)+"]: "+
                           "Set/FillValue mismatch between _ADJUSTED and _ERROR at "+
                           mismatchAdjErr+
                           " levels (of "+prm_adj.length+")");
                  }
                  if (nan > 0) {
            ...
            if (! info.isValid()) {
               formatErrors.add("PROFILE_"+param+"_QC["+(profNum+1)+
                     "]: '"+profQC+"': Invalid");

            } else {
               if (info.isDeprecated) {
                  formatWarnings.add("PROFILE_"+param+"_QC["+(profNum+1)+
                                   "]: '"+profQC+"': Deprecated");
               }
               ...
               if (paramErr  ||  param_adjErr) {
                  ...
                  expProfQC = ' ';
                  if      (n_noqc == n_data) { expProfQC = ' '; }
                  else if (n_good == n_data) { expProfQC = 'A'; }
                  else if (pctGood >= 75.d)  { expProfQC = 'B'; }
                  else if (pctGood >= 50.d)  { expProfQC = 'C'; }
                  else if (pctGood >= 25.d)  { expProfQC = 'D'; }
                  else if (pctGood > 0.d)    { expProfQC = 'E'; }
                  else                       { expProfQC = 'F'; }

                  if (expProfQC != profQC ) {
                     formatErrors.add("PROFILE_"+param+"_QC["+(profNum+1)+
                           "]: Value = '"+profQC+"'. Expected = '"+expProfQC+"'");
                  }
               }//..end if paramErr | param_adjErr
            }
            if (log.isDebugEnabled()) {
               log.debug("PROFILE_"+param+"_QC["+profNum+"]: '"+profQC+
                     "'. Expected: '"+expProfQC+"'");
            }
   ...
   {
      if (log.isDebugEnabled()) { log.debug(".....validateQC....."); }
