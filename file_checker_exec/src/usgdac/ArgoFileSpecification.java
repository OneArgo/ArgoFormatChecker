 ...
 * {@link usgdac.ArgoDataFile#verifyFormat() ArgoDataFile.verifyFormat}
 ...
 * </b>
 * <p>
 * A <i>Template Specification</i> defines the nominal structure of the Argo netCDF
 * files (the dimensions, the variables, and the attributes) and the allowed
 * physical parameters.
 * <p>
 * <b>
 ...
 * </b>
 * <p>
 * A <i>Full Specification</i> defines the structure of the Argo netCDF
 * files (the dimensions, the variables, and the attributes), the allowed
 * physical parameters, the optional parameters, allowed attribute
 * "exceptions", allowed technical parameter names/units, allowed
 * configuration parameter names, and various reference tables.
 * <p>
 * 
 * <b>
 ...
 * Note: <i>Physical parameter</i> variables are <i>not</i> specified
 * in this file.
 * (See <a href="#ParamSpec">"Physical Parameter Specification"</a> below)
 * <p>
 * <i>The format of the specification file is specific:</i>
 * <ul>
 * <li>Blank lines are ignored
 * <li>Everything following comment characters ("//") is ignored
 * <li>Special comment lines that start with "//@" define "constants" 
 * that can be used when this file is used as a template. These "special
 * constants" do not affect the format checking process.
 * Examples of these special constant are:
 * <br><pre><code>
 ...
 *
 * <p>
 * Specifies the elements (dimensions and variables) within the Argo file
 * that are optional.
 * Generally, physical parameters are <i>not</i> included in this file because
 * this is defined in the "Physical Parameter Specification". However, if they
 * do occur in this file, it overrides the "Physical Parameter Specification".
 * <p>
 * Additionally, the variables can be grouped such that all of the variables 
 * in the group must either be omitted or be present.
 * (see HISTORY_INSTITUTION below for example: Either all related variables must
 * be present or all must be missing.)
 * <p>
 ...
 * </b></a><br>
 * File name: argo-<i>&lt;file-type&gt;</i>-spec-v<i>&lt;version&gt;</i>.attr_regexp<br>
 * Optional
 *
 * <p>
 * Specifies allowed exceptions to the attributes (variable and global) defined
 * in the "CDL Specification". The exceptions are specified using 
 ...
 * Required
 * <p>
 * A text file based on the Excel spreadsheet available online at the ADMT website. 
 *
 *
 *
 * <p>
 ...
 *
 * <p>
 * Specifies the allowed names and units of technical parameters.
 * These files are built from the official list of Technical Parameter Names
 * maintained as an Excel file on the ADMT website.
 * <p>
 * Blank lines and comment lines (lines starting with "//") are ignored.
 * <p>
 ...
 * </b></a><br>
 * File names:
 *    argo-bio_config_names-spec-v<i>&lt;version&gt;<br>
 *    argo-core_config_names-spec-v<i>&lt;version&gt;<br>
 *    argo-tech_units-spec-v<i>&lt;version&gt;</i><br>   <-----NOTICE
 * Required
 *
 * <p>
 * Specifies the allowed names and units of configuration parameters.
 * These files are built from the official list of Configuration Parameter Names
 * and Technical Parameter UNITS maintained as an Excel file on the ADMT website.
 * <p>
 * Blank lines and comment lines (lines starting with "//") are ignored.
 * <p>
 * The format of each file is:<br>
 ...
 * Required
 * <p>
 * Assorted reference tables from the Users Manual.
 * The tables required for file checking are: 2, 2a, 5, 6, 8, 9, 10, 16, 19, 20, 22, 23, 24, 25, 26, 27
 * <p>
 * The format used for each file is the same and is "one size fits all".  They are
 * multi-column files but only the first column is necessary for file checking.
 * The columns are separated by the "&brvbar;" character.  Leading and trailing
 * spaces in a column are ignored.
 * <p>
 * The format is:<br>
 * <pre>
 * {@code
 ...
 * VARIABLE GROUPS
 * </b></a><br>
 * A discussion of the "variable groups" is required.
 * It's very non-intuitive but is needed to support the
 * "existence rules" for parameter variables.
 * And may intersect with the OPT specification file.
 * <p>
 ...
 * </ul>
 * <p>
 * The concept of "groups" is introduced to help enforce the
 * rules.  A variable has a "primary group" -- the group that
 * it is primarily associated with.  A variable may also belong
 * to one or more ancillary groups.
 *
 * The basic concept is that when a variable is reported, all
 * members of its primary group must also be reported.
 * <p>
 * CORE- AND BIO- PARAMETERS - THE BASIC RULE:
 * If any one of the variables exist in the file, 
 *      they ALL must be in the file  (easy-peasy)
 * <p>
 * Implementation:
 * <br>Each of the 6 variables has the same primary group == "param-name"
 *     <ul>
 *     <li> Primary group is stored in "Map" varGroup.put(var-name, pri-group)
 *     <li> A group's members are stored in groupMembers
 *          <br>  member-list = groupMembers.get(param-name)
 *     </ul>
 * <p>
 ...
 * <li> "qc-variables"
 *      <ul>
 *      <li> These 2 variables have a "primary group" of PARAM_qc
 *           <br> varGroup.get("qc-variable")   returns PARAM_qc group
 *      <li> Group: PARAM_qc  has members PARAM and (2) "qc variables"
 *           <br> goupMembers.get(PARAM_qc)   returns PARAM and qc-variables
 *      </ul>
 * <p>
 * <li> "adj-variables"
 *      <ul>
 *      <li> These 3 variable have a "primary group" of PARAM_adj
 *           <br> varGroup.get("adj-variable")  returns PARAM_adj group
 *      <li> Group: PARAM_adj  contains all variables (PARAM, qc- and adj-variables)
 *           <br> goupMembers.get(PARAM_adj)  returns PARAM, qc-, and adj-variables
 *      </ul>
 * <p>
 * (((A note for future reference:  If a param-variable does not have a primary group
 * then it can exist by itself.  (This was once allowed for PARAM.) In this case,
 *      <ul>
 *      <li> It doesn't know it belongs to a group
 *          <br>   varGroup.get(PARAM)      returns null
 *          <br>   groupMembers.get(PARAM)  returns null
 *      </ul>
 * )))
 * <p>
 * <li>PARAM_stat (*_STD, *_MED) variables:
 *     <br> (currently not allowing PROFILE_*_QC, *_QC variables
 *     <br> PARAM_stat           no primary group (it's independent)
 *     <br> PARAM_stat_ADJUSTED  
 * </ul>
 ...
 * @version  $HeadURL: https://inversion.nrlmry.navy.mil/svn/godae/trunk/argo/bin/java/usgdac/ArgoFileSpecification.java $
 * @version  $Id: ArgoFileSpecification.java 1323 2022-04-19 16:13:39Z ignaszewski $
 */
public class ArgoFileSpecification
{
   ...
   private static final HashMap<String, ArgoDataFile.FileType> paramFileTypes =
      new HashMap<String, ArgoDataFile.FileType>();
   ...
   /**
    * When an attribute specification starts with this string, the
    * attribute is ignored during file checking.  The attribute can be
    * present, or not, and its value is ignored.
    * The attribute is part of the specification, technically.
    */
   public static final String ATTR_IGNORE = "<*>"; //..present, or not, ignore value

   /**
    * When an attribute specification starts with this string, the
    * attribute <b>value</b> is ignored during file checking.  The attribute must
    * be present but its <b>value</b> is ignored.
    * The attribute is part of the specification, technically.
    */
   ...
   /**
    * When an attribute specification starts with this string, the
    * attribute is <b>not allowed</b> in the data file.
    * Since "extra attributes" are ignored, this is a method to force
    * an attribute to be excluded.
    */
   ...
   private static final String comment_on_resolution = 
      new String("comment_on_resolution");
   ...
   static
   {
      ...
      //..group 1: variable:attribute; group 2: variable; group 3: attr; group 4: regex
      ...
      pGlblAttr = 
         Pattern.compile("^\\s*:(\\w+)\\s*=\\s*(?:\"(.*)\"\\s*|(.*));\\s*(?:/\\*\\s*REGEX\\s*=\\s*\"(.*)\"\\s*\\*/)*\\s*");
      ...
      //..group 1: variable name; group 2: attribute name; group 3/4: string/number definition
      pVarAttr = 
           Pattern.compile("^\\s*(\\w+):(\\w+)\\s*=\\s*(?:\"(.*)\"\\s*|(.*));\\s*");
...
//............................................
   ...
   /**
    * Builds the Argo file specification for the specified type and version
    * from the specification files in the specfication directory
    *
    ...
    */
   public ArgoFileSpecification
      (boolean fullSpec, String specDir, ArgoDataFile.FileType fType, String version)
      throws IOException
   {
   ...
   /**
    * Adds an "extra dimension" to the specification.
    * These are dimensions added to allow for an extra data dimension in the
    * physical parameters.
    *
    * The name of the dimension must:
    * 1) match an "extra dimension" pattern in the CDL spec file
    * 2) not already exist in the specification
    *
    * @param name the string name of the requested dimension
    ...
    * @return The string names.  Null if not defined.
    */
   public String getMeta(String name) { return metaHash.get(name); }
    ...
    */
   public String getSpecName() { return new String(specName); }
    ...
    * @return True - name is valid physical parameter name;
    *         False - parameter name is NOT valid (or spec not opened)
    *        
    */
    ...
    */
   public void openSpecification (boolean fullSpec, String specDir, 
                                  ArgoDataFile.FileType fType, String version)
      throws IOException
   {
   ...
   //.............................................
   /**
    * Parses a CDL file.  The file components are accessed through the
    * various accessors.
    *
   ...
   public boolean parseCdlFile() throws IOException
   {
      ...
      if (! f.isFile()) {
         log.error("cdlFileName '" + 
                      cdlFileName + "' does not exist");
         throw new IOException("cdlFileName ('" + cdlFileName + 
                               "') does not exist");
      } else if (! f.canRead()) {
         log.error("cdlFileName '" + 
                      cdlFileName + "' cannot be read");
         throw new IOException("cdlFileName ('" + cdlFileName + 
                               "') cannot be read");
      }
      ...
      while ((line = file.readLine())  != null) {
         ...
         if (mCmt.find()) {
            ...
            if (mCmt.group(1) != null) {
               log.debug("special comment: '{}'  '{}'",
                         mCmt.group(1), mCmt.group(2));
         ...
         if (pBlankOrComment.matcher(line).matches()) {
            ...
            if (log.isInfoEnabled()) {log.info("spec Name: '" + specName + "'");}
            ...
            log.debug("***  VARIABLES  ***");
                    ...
                    pCloseTag.matcher(line).matches()) {
   ...
   private boolean parseDimLine (String line)
   {
      ...
      if (m.matches()) {
         ...
         try {
            ...
            message = "Extra dimension is not a valid regular expression: '"+
               name+"'";
            return false;
         }
    ...
    * <p>
    * NOTE: Currently only expects string valued attributes, but
    * the plumbing is there to handle numeric types too.
    * @param line the dimension line to parse
    */

   private void parseGlblAttrLine (String line)
      throws IOException
   {
      ...
      if (match) {
         log.debug("parseGlblAttrLine: groups = '{}' '{}' '{}' '{}'",
                   m.group(1), m.group(2), m.group(3), m.group(4));
   ...
   private boolean parseVarLine (String line)
   {
      ...
      ArgoVariable var = (ArgoVariable) null;
      ...
      if (isVar) {
         ...
         if (log.isDebugEnabled()) {
            log.debug("variable: name = '" + name + "'  type = '" +
                      type + "'   dim = '" + dim + "'");
         }
         ...
         if (dim != null) {
            ...
            while (mDim.find()) {
               if (mDim.group(2) != null) {
                  ...
                  if (d == (ArgoDimension) null) {
                     message = "Dimension used ('"+s+
                  ...
                  if (d == null) {
                     ...
                     for (String alt : s.split("\\|")) {
                        if (dimHash.get(alt) == null) {
                           message = "Alternate dimension name '"+alt+
         ...
         log.debug("attribute: '"+varName+"'  '"+name+"'  '"+strValue+
                   "'  '"+numValue+"'");
         ...
         log.error("'"+line.trim()+
                   "' is not identified as isVar() or isAttr()");
   ...
   //......................................................
   /**
    * Parses the "optional element" file of a specification.
    * This file defines which elements in the CDL specification are optional.
    *
   ...
   public boolean parseOptFile () throws IOException
   {
      ...
      if (! f.canRead()) {
         log.error("optFileName '" + 
                   optFileName + "' exists but cannot be read");
         throw new IOException("optFileName ('" + optFileName + 
   ...
   //............................................................
   /**
    * Parses the "attribute regex" file of a specification.
    * Defines regular expressions used to match the specified
    * reqular expressions (global or variable).
    * <p>
   ...
   public boolean parseAttrRegexFile ()
      throws IOException
   {
      String line;
      ...
      if (! f.canRead()) {
         log.error("regFileName '" + 
                   regFileName + "' exists but cannot be read");
         throw new IOException("regFileName ('" + regFileName + 
                                  "') exists but cannot be read");
      }
      ...
      while ((line = file.readLine()) != null) {
         ...
         if (pBlankOrComment.matcher(line).matches()) {
            ...
            log.debug("variable:attribute, var, attr, regex, warn = '{}', '{}', '{}', '{}', '{}'", 
                      var_attr, var, attr, regex, warn);

            if (regex.length() == ATTR_SPECIAL_LENGTH && 
                regex.matches(ATTR_SPECIAL_REGEX)) {
    ...
    *
    * Reads two files:
    *    prmFileName    - the primary list of physcial parameter names
    *    prmFileNameAux - contains "auxilliary" setting for certain parameters
    *
    * @param fileType The allowed physical parameters are dependent on the file type.
    * @param version Version of the specification
    * @param listOnly true - only make a list of ALL parameter names; 
    *                 false - build the variable structure for the specification
    *                         (based on file type)
    * @return True - file parsed; False - failed to parse file
    * @throws IOException indicates file read or permission error
    */

   public boolean parseParamFile (ArgoDataFile.FileType fileType,
                                  String version, boolean listOnly)
      throws IOException
   {
      log.debug(".....parseParamFile: start.....");
      log.debug("fileType, version: '{}' '{}' --- listOnly {}",
                fileType, version, listOnly);

      //..check file type and build the appropriate dimension array

      ArgoDimension dimPQc[] = new ArgoDimension[1];
      ArgoDimension dimParam[] = (ArgoDimension []) null;
      ...
      String errComment = (String) null;
      String errLongName = (String) null;
      ...
      if (! f.isFile()) {
         ...
         throw new IOException("prmFileName ('" + prmFileNameAux + 
                               "') does not exist");
      } else if (! f.canRead()) {
      ...
      if (! f.isFile()) {
         ...
         throw new IOException("prmFileName ('" + prmFileName + 
                               "') does not exist");
      } else if (! f.canRead()) {
         log.error("prmFileName '" + prmFileName + "' cannot be read");
         throw new IOException("prmFileName ('" + cdlFileName + 
                               "') cannot be read");
      }
      ...
      while ((line = file.readLine()) != null) {
         ...
         if (version.compareTo("3.0") > 0) {
            ...
            if (prmLName.length() == 0  || prmLName.equals("-")) prmVmax = ATTR_IGNORE;
         }

         log.debug("parsed: {}|{}|{}|{}|{}|{}|{}|", 
                   prm, prmType, prmLName, prmSName, prmUnits, 
                   prmVmin, prmVmax, prmStatus, prmFill);
         ...
         String tmp = prmType;
         switch (prmType) {
         case "NC_DOUBLE": prmType = "double"; break;
         case "NC_FLOAT":  prmType = "float"; break;
         case "NC_SHORT":  prmType = "short"; break;
         }
         ...
         if (! (prmType.equals("float") || prmType.equals("double") || prmType.equals("short")) ) {
            ...
            throw new IOException("Invalid data type '"+prmType+"' ("+prmFileName+
                                  "; "+nLine+")");
         }
         ...
         if (prmFill.startsWith("NC_FILL_")) {
            ...
            if (prmFill == null) {
               ...
               throw new IOException("Invalid NC_FILL_: '"+tmp+"' ("+prmFileName+
                                  "; "+nLine+")");
            }
         ...
         //..check the PrmCat -- "c" = core, "b" = bio, "ic/ib" = intermediate -> core, bio
         if (! (prmCategory.equals("c")  || prmCategory.equals("b") || 
                prmCategory.equals("ic") || prmCategory.equals("ib")) ) {
            log.debug("Invalid category '"+prmCategory+"' in line '"+line+"'");
            throw new IOException("Invalid category '"+prmCategory+"' ("+prmFileName+
                                  "; "+nLine+")");
         }
            ...
            if (keep) {
               ...
               if (fileType == ArgoDataFile.FileType.PROFILE ||
                  ...
                  if (! bio_pres) {
                     ...
                     log.debug("added: '{}'; opt '{}'; primary '{}'; member '{}'",
                               varNm, opt, group, group);
               ...
               for (String v : P) {
                  /* Build the parameter structure
                   * Profile:
                   *  <ncDataType> <P>(N_PROF, N_LEVELS) -or- <ncDataType> <P>(N_PROF, N_LEVELS,_extra_)
                   *
                   * Trajectory:
                   *  <ncDataType> <P>(N_MEASUREMENT);  -or-  <ncDataType> <P>(N_MEASUREMENT, _extra_)
                   *
                   *  Attributes:
                   *       <P>:long_name = "<param-specific>";
                   *       <P>:standard_name = "<param-specific>";
                   *       <P>:_FillValue = <param-specific>;
                  ...
                  addAttr(aVar, comment_on_resolution, 
                          ATTR_IGNORE + "resolution is unknown", DataType.STRING);
                  ...
                  /* Do not put PRES_ADJUSTED in bio-argo file
                   * -- bio_pres = true ==>  <PARAM> = PRES  &&  this is a bio-profile file
                   */
                  if (bio_pres) {
               ...
               /* Build the parameter structure
                *  Profile
                *    <ncDataType> <PARAM>_ADJUSTED_ERROR(N_PROF, N_LEVELS);
                *  Trajectory
                *    <ncDataType> <PARAM>_ADJUSTED_ERROR(N_MEASUREMENT);
                *
                ...
                *       <PARAM>_ADJUSTED_ERROR:comment = "Contains the error on the \
               ...
               if (! bio_pres) {
                  ...
                  addAttr(aVar, long_name,
                          (errLongName == (String) null ? prmLName:errLongName), DataType.STRING);
                  ...
                  varGroup.put(varNm, group);
                  groupMembers.get(group).add(varNm);
                  log.debug("added: '{}'; opt '{}'; primary group'{}'; member '{}'",
                            varNm, opt, group, group);
               ...
               /* ****************************************************************
                * STAT variables: Every parameter can have a 
                *    <PARAM>_STD and/or <PARAM>_MED group
                * including the *_ADJUSTED variables --- all optional
                *
                * The STAT variables are only allowed in the file of the primary parameter
                *    core param STAT only in core files
                *    bio param STAT only in bio files
                *
                */
               ...
               if (! bio_pres) {
                  ...
                  for (String statNm : STAT) {
                     ...
                     /* ***** Apr 2015
                      * Users of the STAT variables are not including PROFILE_*_QC
                      * So don't allow them at all until there is some outcry
                      *
                      * ***** Update: Apr 2022
                      * Handle these as "intermediate parameters"
                      */
                     ...
                     for (String v : S) {
                        ...
                        /* Build the parameter STAT structure
                         * Profile:
                         *  <ncDataType> <S>(N_PROF, N_LEVELS); 
                         *    -or- <ncDataType> <S>(N_PROF, N_LEVELS,_extra_)
                         * Trajectory:
                         *  <ncDataType> <S>(N_MEASUREMENT);
                         *    -or- <ncDataType> <S>(N_MEASUREMENT, _extra_)
                         *   <S>:long_name = "<param-specific>";
                         *   //<S>:standard_name = "<param-specific>";
                         *   <S>:_FillValue = <param-specific>;
                         *   <S>:units = "<param-specific>";
                         *   <S>:valid_min = <param-specific>;
                         *   <S>:valid_max = <param-specific>;
                         *   <S>:comment = "<param-specific>";  (version-specific)
                         *   <S>:C_format = ATTR_IGNORE_VALUE;   //..means ignore setting
                         *   <S>:FORTRAN_format = ATTR_IGNORE_VALUE; //..means ignore setting
                         *   <S>:resolution = ATTR_IGNORE_VALUE; 
                         *   <S>:comment_on_resolution = ATTR_IGNORE; ///..means ignore entirely
                         *           only in v3.1 and beyond (but technically would
                         *           be allowed in earlier versions if it showed up
                        ...
                        /* Build the parameter structure
                         *    char <S>_QC(N_PROF, N_LEVELS);
                         *       <S>_QC:long_name = "quality flag";
                         *       <S>_QC:conventions = "Argo reference table 2";
                         *       <S>_QC:_FillValue = " ";
                         *
                         * where <S> = <STAT>, <STAT>_ADJUSTED
                         */
                     ...
                     /* Build the parameter structure
                      *    <ncDataType> <STAT>_ADJUSTED_ERROR(N_PROF, N_LEVELS);
                      *       <STAT>_ADJUSTED_ERROR:long_name = "<param-specific>";
                      *       <STAT>_ADJUSTED_ERROR:_FillValue = <param-specific>;
                      *       <STAT>_ADJUSTED_ERROR:units = "<param-specific>";
                      *       <STAT>_ADJUSTED_ERROR:comment = "Contains the error on the \
                      *           adjusted values as determined by the delayed mode QC process.";
    ...
    * @param attVal Attribute value -- can be the attribute "special codes"
    *               Input as a string then converted to the appropriate type
   ...
   private void addAttr (ArgoVariable var, String attrName, String attrVal, DataType type) {
      ...
      if (attrVal.matches(ATTR_SPECIAL_REGEX)) {
         ...
         if (tmp.length() == 0) {
            ...
            log.debug("addAttr: (special) attrName, sa, type, def = '{}', '{}', '{}', (no default)",
                   attrName, sa, type);
            ...
            log.debug("addAttr: (special) attrName, sa, type, def = '{}', '{}', '{}', '{}'",
                   attrName, sa, type, tmp);
         }

      } else if (type == DataType.STRING) {
         var.addAttribute(attrName, attrVal);

         log.debug("addAttr: (string) attrName, attrVal = '{}', '{}'",
                   attrName, attrVal);
         ...
         log.debug("addAttr: (number) attrName, attrVal, num = '{}', '{}', '{}'",
                   attrName, attrVal, num);
      }
