 ...
 *
 * Use <b>open</b> to open an existing file and create an ArgoDataFile object
 * of the proper type.
 * <p>
 * To open a new Argo data file; use the class appropriate for the
 * specific type of file you are opening (meta, profile, trajectory, technical).
 * <p>
 * To be successfully opened, the <i>Argo Data File</i> must be a netCDF
 * file with the variables DATA_TYPE and FORMAT_VERSION defined 
 * with expected (Argo) values.  (At the initial opening, the routines are not
 * very demanding of the format.)  If there is any doubt as to the validitly
 * of the file format, it is highly recommended that the file format be
 * explicity checked.
 * <p>
 * To ensure the file is a valid <i>Argo Data File</i>, open the file
 * with the directory for the "specification" provided (or open
 * the <i>specification</i> separately) and call <b>verifyFormat</b>.
 * <p>
 * <b>FORMAT VERIFICATION:</b><br>
 * The format verification process (<i>verifyFormat</i>) compares the
 * data file to the "specification".  Details of the specification are
 * documented in {@link usgdac.ArgoFileSpecification}.
 * .
 * <p>
 ...
 * <li>The variables in the data file match the specification
 * (name, type, and dimensions)
 * <li>The attributes of the variables match (name and setting)
 * </ul>
 * <p>
 * The specification includes two additional features: 
 * optional parameters and highly-desirable parameters.
 * <p>
 * Optional parameters: 
 * These are dimensions and variables that may be omitted from a data file.
 * These are generally parameters that are not measured by all platforms (such
 * as CNDC and DOXY).  Additionally, the variables can be grouped into parameter
 * groups such that all of the variables in the group must either be omitted
 * or present.  (For instance, for DOXY either all DOXY-related variables must
 * be present or missing.)
 *
 * Highly-desirable parameters:
 * The specification also includes <i>highly-desirable</i> parameters.
 * These are variables that are expected to have values with certain settings.
 * Failure to match these only generates a format-warning.
 ...
 * <p>
 * A set of convenience routines are provided to read data values from an argo file.
 * These anticipate "reads" based on the format of the data files.
 * <ul>
 ...
 * @version  $HeadURL: https://inversion.nrlmry.navy.mil/svn/godae/trunk/argo/bin/java/usgdac/ArgoDataFile.java $
...
//................................................
   ...
   public static enum FileType { 
      ...
      FileType(String s) { specType = s; }
   };

   protected static String message = BLANK_MESSAGE;

   protected static HashMap<String,ArgoFileSpecification> fullSpecCache = 
	   new HashMap<String,ArgoFileSpecification>();
   protected static HashMap<String,ArgoFileSpecification> tmpltSpecCache = 
	   new HashMap<String,ArgoFileSpecification>();

   protected static SimpleDateFormat stringDateFormat = 
...
//.........................................
   ...
   public void clearFormatErrors() {
      if (formatErrors != null) formatErrors.clear();
   }

   /** Clear the warnings */
   public void clearFormatWarnings() { 
      if (formatWarnings != null) formatWarnings.clear();
   }

   /** Retrieve the NetcdfFile reference */
   public String getFileName () { return ncFileName; }
   
   /** Retrieve file type **/
   public FileType fileType() { return fileType; }

   /** Retrieve the file version */
   public String fileVersion() { return new String(format_version); }
...
//.........................................
   ...
   /**
    * Determines if a float value is the specified FillValue
    * Performs a "real value" comparison to a resolution of
    * 1.e-5 (not an exact equality).
   ...
   /**
    * Determines if a double value is the specified FillValue
    * Performs a "real value" comparison to a resolution of
    * 1.e-5 (not an exact equality).
    * @param _FillValue for the parameter
    * @param data value
   ...
   /**
    * Determines if a float value is a "99,999.0" missing value.
    * Performs a "real value" comparison to a resolution of
    * 1.e-5 (not an exact equality).
    * <br>
    * <br>NOTE: This is required because we've run into examples
    * where "bad values" of a parameter have been larger than the FillValue
    * so simple greater-than/less-than tests fails
    * @param data value
    * @return true = value is FillValue; false = value is not FillValue
    */
   ...
   /**
    * Determines if a double value is a "99,999.0" missing value.
    * Performs a "real value" comparison to a resolution of
    * 1.e-5 (not an exact equality).
    * <br>
    * <br>NOTE: This is required because we've run into examples
   ...
   /**
    * Determines if a double value is a "999,999.0" missing value.
    * Performs a "real value" comparison to a resolution of
    * 1.e-5 (not an exact equality).
    * @param data value
    * @return true = value is FillValue; false = value is not FillValue
    ...
    */
   public static boolean canOpen (String inFile)
      throws IOException
   {
   ...
   //.............open (inFile).................
   /**
    * Opens an existing file (without opening the assoicated <i>Argo specification</i>).
    *
    * @param inFile the string name of the file to open
    * @param dacName Optional name of the dac that owns the file
    * @return the file object reference. Returns null if the file is not an Argo file.
    * @throws IOException If an I/O error occurs
    */
   public static ArgoDataFile open (String inFile, String... dacName)
      throws IOException
   {
      try {
   ...
   /**
    * Opens an existing file (without opening the assoicated <i>Argo specification</i>) and, optionally,
    * ignores a bad DATA_TYPE --- will still detect accepted non-standard settings in
    * old v3.1 files.
    *
    * @param inFile the string name of the file to open
    ...
    * @throws IOException If an I/O error occurs
    */
   public static ArgoDataFile open (String inFile, boolean overrideBadTYPE, String... dacName) 
      throws IOException
   {
      ...
      /*....................................................
       *....these are exceptions currently being allowed....
       *....................................................*/
         ...
         log.warn("TEMP WARNING: {}: {}: {}", dac, inFile,
                  "Non-standard DATA_TYPE (temporarily allowed): '" + dt + "'");
         badtype = 
         "DATA_TYPE = 'ARGO trajectory' non-standard. Change to 'Argo trajectory'";
         ft = FileType.TRAJECTORY;
         ...
         log.warn("TEMP WARNING: {}: {}: {}", dac, inFile,
                  "Non-standard DATA_TYPE (temporarily allowed): '" + dt + "'");
         badtype = 
         "DATA_TYPE = 'ARGO technical' non-standard. Change to 'Argo technical data'";
         ft = FileType.TECHNICAL;
         ...
         log.warn("TEMP WARNING: {}: {}: {}", dac, inFile,
                  "Non-standard DATA_TYPE (temporarily allowed): '" + dt + "'");
         badtype = 
         "DATA_TYPE = 'ARGO technical data' non-standard. Change to 'Argo technical data'";
         ft = FileType.TECHNICAL;

      /*....................................................
       *....above are exceptions currently being allowed....
       *....................................................*/
         ...
         ft = FileType.UNKNOWN;
         message = new String("Invalid DATA_TYPE: '" + dt + "'");
         return (ArgoDataFile) null;
      }
      ...
      ArgoDataFile arFile = (ArgoDataFile) null;
      ...
      arFile.fileType = ft;
      arFile.formatErrors = new ArrayList<String>();
      arFile.formatWarnings = new ArrayList<String>();
      arFile.format_version = fv;

      if (! overrideBadTYPE) {
         if (badtype != (String) null  &&  fv.trim().equals("3.1")) {
            log.info("Invalid DATA_TYPE: '" + dt + "'");
            ft = FileType.UNKNOWN;
            message = new String("Invalid DATA_TYPE: '" + dt + "'");
            return (ArgoDataFile) null;
    ...
    *
    * @param inFile The string name of the file to open
    * @param specDir The string name of the directory containing the specification files
      ...
      throws IOException
   {
      ...
      if (arFile == (ArgoDataFile) null) {
         return arFile;
      }

      //..create the specification
      try {
         arFile.spec = openSpecification(fullSpec, specDir, arFile.fileType, 
                                  arFile.format_version);
      } catch (IOException e) {
         if (e.getMessage().matches("cdlFileName.*does not exist")) {
            message = "File type / version not valid in the FileChecker: "+
               arFile.fileType+" / "+arFile.format_version;
            return ((ArgoDataFile) null);
         } else {
            throw e;
         }
   ...
   {
      if (ncReader != null) ncReader.close();
      file = (File) null;
      ncFileName = (String) null;
   ...
   //...............openSpecification...................
   /**
    * Opens the <i>Argo specification</i> associated with this Argo file.
    * An <i>Argo specification</i> is a description of the official Argo file format.
    * @see usgdac.ArgoFileSpecification ArgoFileSpecification
    *
   ...
   public static ArgoFileSpecification openSpecification 
      (boolean fullSpec, String specDir, FileType ft, String version)
      throws IOException
   {
   ...
   private boolean ckVarAttr (Variable dataVar, ArgoVariable specVar)
   {
      ...
      for (Attribute dataAttr : dataVar.getAttributes()) {
         ...
         if (specAttr == (ArgoAttribute) null) {
             ...
             *
             *       returnVal = false;
             *       formatErrors.add("attribute: '"+varName+":"+
             *                        attrName+"' not defined in specification '"+
             *                        spec.getSpecName()+"'");
             *
             * For now... they will be ignored by just doing nothing about it
             */

             log.info("extra attribute (allowed): {}:{} not in spec",
                      varName, attrName);
         ...
         if (specialHandling == ArgoAttribute.AttrHandling.NOT_ALLOWED) {
            String err = String.format("attribute: %s:%s: Attribute is not allowed.",
                                       varName, attrName);
         ...
         if (specAttr.isString()) {
            if (! dataAttr.isString()) {
               String err = String.format
                  ("attribute: %s:%s: Incorrect attribute value type. Must be string",
                   varName, attrName);
               //formatErrors.add(err)
            ...
            if ( ! dataAttrType.isNumeric()) {
               String err = String.format
                  ("attribute: %s:%s: Incorrect attribute value type. Must be numeric",
                   varName, attrName);
         ...
         if (! (specialHandling == ArgoAttribute.AttrHandling.IGNORE_COMPLETELY ||
            ...
            if (dataAttr.isString()){
               try {
                  ...
                  formatErrors.add("attribute: "+varName+":"+
                                   attrName+": Bad value.  Not a string.");
                  returnVal = false;
                  continue DATA_ATTR_LOOP;
               }
               ...
               try {
                  ...
                  formatErrors.add("attribute: "+varName+":"+
                                   attrName+": Bad value.  Not a numeric value.");
                  returnVal = false;
                  continue DATA_ATTR_LOOP;
               }
            }
            ...
            if (! dataAttrValue.equals(specAttrValue)) {
               ...
               if (regex == null) {
                  ...
                  if (! regex.pattern.matcher(dataAttrValue).matches()) {
                     formatErrors.add("attribute: "+dataVar.getShortName()+":"+
                                      dataAttr.getShortName()+": Definitions differ "+
                                      "\n\tSpecification = '"+regex.pattern+"' (regex)"+
                                      "\n\tData File     = '"+dataAttrValue+"'");

                     log.info("format error: "+attrName+
                              " attribute regex mismatch '"+regex.pattern+"'");

                     returnVal = false;
                  } else {
                     if (regex.warn) {
                        formatWarnings.add("attribute: "+dataVar.getShortName()+":"+
                           dataAttr.getShortName()+": Accepted; not standard value"+
                           "\n\tSpecification     = '"+specAttrValue+"'"+
                           "\n\tException allowed = '"+regex.pattern+"' (regex)"+
                           "\n\tData File         = '"+dataAttrValue+"'");
                        log.warn("regex match (WARN): attribute '{}:{} = '{}' matches '{}'", 
                                 varName, attrName, dataAttrValue, regex.pattern);
                     } else {
                        log.warn("regex match (NO WARN): attribute '{}:{} = '{}' matches '{}'", 
                                 varName, attrName, dataAttrValue, regex.pattern);
                     }
                  }
               } //..end if regex
   ...
   private boolean ckVarDims (Variable dataVar, ArgoVariable specVar) 
   {
      ...
      log.debug
      ("ckVarDims: pass = {}: specRank, dataRank, altDimVar, exDimVar = {}, {}, {}, {}",
       pass, specRank, dataRank, altDimVar, exDimVar);
      ...
      if (! pass) {
         if (exDimVar) {
            ...
            log.info("format error: '{}' dimensions mismatch (extra dimension)", 
                     dataVar.getShortName());
            ...
            log.info("format error: '{}' dimensions mismatch", 
                     dataVar.getShortName());
         }
      }
   ...
   private boolean ckVarTypes (Variable dataVar, ArgoVariable specVar) 
   {
      ...
      if (specType.equals(dataType)) {
         ...
         if (specVar.getType() == DataType.OPAQUE &&
             (dataType == DataType.FLOAT || dataType == DataType.DOUBLE)) {
            return true;
         }

         formatErrors.add("variable: "+dataVar.getShortName()+": Definitions differ"+
                          "\n\tSpecification type = '"+specType.toString()+"'"+
   ...
   //.......................verifyFormat.....................
   /**
    * Verifies whether this file conforms to the Argo Netcdf specification.
    * The specification for this file type and version must be open.
    * (See 
    * @see usgdac.ArgoFileSpecification
    * )
    * <p>
    * <ul>
   ...
   {  
      if (spec == (ArgoFileSpecification) null) {
         log.info("File specification not opened");
         message = "ERROR: File specification not opened for this file";
         formatErrors.add("ERROR: File specification not opened for this file");
         return false;
      }

      /*
       * METHOD:
       * 1) Iterate through the data file dimensions and compare to the spec
       *    - if an optional "extra" dimension is encountered, add it to the spec
       * 2) Iterate through the data file variables and compare to the spec
       * 2a) - for each variable, iterate through the attributes and compare
       *    - attribute regexs??????
       * 3) Iterate through the spec dimensions and compare to the data file
       * 4) Iterate through the spec variables and compare to the data file
       *    - postpone processing of missing optional variables
       * 4a) - for each variable, iterate through the attributes and compare
       * 5) For missing optional variables, check that all variables for the
       *    group are missing.
       * 6) Iterate through the spec global attributes and compare to the data file
       * 7) Clear any "extra" dimensions that were added
       */
         ...
         for (Dimension dataDim : dimList) {
            ...
            if (specDim == (ArgoDimension) null) {
               ...
               if (specDim == null) {
                  ...
                  formatErrors.add("dimension: "+dimName+
                     ": not defined in specification '"+spec.getSpecName()+"'");

                  log.info("format error: '{}' not in spec", dimName);


               } else {
                  log.debug("extra dimension: '{}'. value = {}", 
                            dimName, dataDim.getLength());
               }
               ...
               if (specValue > 0 && specValue != dataValue) {
                  ...
                  log.info("format error: '{}' dimension value mismatch", 
                           dataDim.getShortName());
               }
      ...
      for (Variable dataVar : varList) {
         ...
         if (specVar == (ArgoVariable) null) {
            //..data file variable is not in the specification
            formatErrors.add("variable: "+name+
                       ": not defined in specification '"+spec.getSpecName()+"'");
      ...
      for (ArgoDimension dim : spec.getDimensions()) {
         ...
         if (dataDim == (Dimension) null) {
            if (spec.isOptional(name)) {
               //..dimension is optional
               log.debug("optional dimension '{}': not defined in data file - allowed",
                         name);
      ...
      for (String name : spec.getSpecVariableNames()) {
         ...
         //..........Step 4a: Check the spec attributes against the data file.......
            ...
            //..- don't have to check values because the data var attr values were checked above
            ...
            for (String attrName : specVar.getAttributeNames()) {
               ...
               if (handling == ArgoAttribute.AttrHandling.IGNORE_COMPLETELY ||
                  ...
                  if (dataAttr == (Attribute) null) {
                     //..attribute in spec file is not in the data file

                     formatErrors.add("attribute: '"+name+":"+attrName+
                                      "' not defined in data file");

                     log.info("format error: attribute not in data file: '{}:{}'",
                              name, attrName);
                  }
               }
            }
      ...
      //.. - for each group with a reported variable -- make sure all of them are reported
      ...
      for (String group : dataGroup) {
         ...
         if (vMiss.size() > 0) {
            ...
            //..add to formatErrors
            formatErrors.add("Parameter group "+group+
                                     ": Variables are missing for this group"+
                       "\n\tRequired variables: "+req+
                       "\n\tMissing variables:  "+miss);

            log.info("format error: option group '{}' variables missing from data file",
                      group);
         }
      }
      ...
      for (String name : spec.getGlobalAttributeNames()) {
         ...
         if (log.isDebugEnabled()) { log.debug("spec attribute: "+name); }
         ...
         if (dataAttr == (Attribute) null) {
            ...
            if (dataAttr.isString()) {
                        ...
                        if (! regex.pattern.matcher(dataValue).matches()) {
                           ...
                           if (regex.warn) {
                              formatWarnings.add("global attribute: "+name+
                                         ...
                                         "\n\tException allowed = '"+regex.pattern+"' (regex)"+
                                         "\n\tData File         = '"+dataValue+"'");
                              log.warn("regex match (WARN): global attribute ':{} = '{}' matches '{}'", 
               ...
               String err = String.format("global attribute: %s: not a \"string valued\" attribute",
                                          name);
      ...
      if (formatErrors.size() == 0) { verified = true; }

      if (log.isDebugEnabled()) { log.debug(".....verifyFormat: completed....."); }
      return true;
   } //..end verifyFormat
   ...
   public String getGdacFileName ()
   {
      ...
      if (fileType == FileType.METADATA) {
         ...
         log.debug("meta-data file: platform, gdacName = '{}', '{}'", 
                   platform, gdacName);
         ...
         if (fileType == FileType.PROFILE) {
            ...
            if (data_mode == 'R' ||  data_mode == 'A') {
               ...
               formatErrors.add("Could not determine file name: invalid DATA_MODE ='"+
                                data_mode+"'");
               return null;
            }
         ...
         if (direction == 'D') {
            ...
            //..do some rudimentary data checks here
            formatErrors.add("Could not determine file name: invalid DIRECTION ='"+
                             direction+"'");
            return null;
         }
         ...
         log.debug("tech-data file: platform, gdacName = '{}', '{}'", 
                   platform, gdacName);
         ...
         log.debug("{} file: platform, data_mode = "+
                   "'{}', '{}'", 
                   fileType, platform, data_mode);

         gdacName.append(platform.trim());
         gdacName.append("_");

         if (fileType == FileType.BIO_TRAJECTORY) gdacName.append("B");
         ...
         formatErrors.add("Could not determine file name: unexpected file type = '"+
                          fileType+"'");
   ...
   public boolean validateGdacFileName ()
   {
      ...
      if (! name.equals(expected)) {
         ...
         formatErrors.add("Incorrect file name\n\tDAC file name:       '"+name+
                          "'\n\tFile name from data: '"+expected.toString()+"'");
         log.debug("validateGdacFileName: FAILED: expected, name = '{}', '{}'", 
                expected, name);

         return false;
      }

      log.debug("validateGdacFileName: PASSED: expected, name = '{}', '{}'", 
                expected, name);
      return true;
   }
   ...
   public void validateStringNulls () throws IOException
   {
      ...
      if (log.isDebugEnabled()) { log.debug(".....validateStrings....."); }
            ...
            if (rank == 1) {
               ...
               for (int i = 0; i < shape[0]; i++) {
                  ...
                  if (ch.getChar(ndx) == nullChar) {
                     formatWarnings.add(var.getShortName()+": NULL character at ["+
                                        (i+1)+"]");
               ...
               for (int i = 0; i < shape[0]; i++) {
                  ...
                  for (int j = 0; j < shape[1]; j++) {
                     ndx.set(i,j);
                     if (ch.getChar(ndx) == nullChar) {
                        formatWarnings.add(var.getShortName()+": NULL character at ["+(i+1)+
                              ","+(j+1)+"]");
                        log.warn("warning: {}[{},{}]: null character", 
                                 var.getShortName(), i, j);
                        break;
                     }
                  }
               }
   ...
   //.....................................................
   /**
    * Performs rudimentary date checks on a file.  Essentially, just checks
    * that DATE_CREATE and DATE_UPDATE are valid date strings.
    * 
    * <p>Upon completion <i>obj</i>.nFormatErrors(), <i>obj</i>.nFormatWarnings, 
    * <i>obj</i>.formatErrors(), and <i>obj</i>.formatWarnings will return results.
    *
   ...
   private int getDimensionLength (NetcdfFile ncReader, String dimName)
   {
      ...
      if (dim == null) {
         message = "Dimension '"+dimName+"' not in Argo data file.";
         return -1;
      }
   ...
   /**
    * Reads a String variable from this netCDF file.
    * NOTE: The variable must be a char[str-length] array.
    * @param varName the string name of the variable to read
    * @param returnNulls (OPTIONAL) true = return NULLS in string;
    *                               false (default) = truncate string if NULL is detected.
    * @return the value of the variable.
    *         null if not read (variable not present or a read error)
    */
   ...
   /**
    * Reads a variable from an open netCDF file
    * NOTE: The variable must be a char[str-length] array.
    ...
    *         null if not read (variable not present or a read error)
    */
   private static String readString (NetcdfFile ncReader, String varName,
                                     boolean... returnNulls)
   {
      Variable ncVar = ncReader.findVariable(varName);
      if (ncVar == (Variable) null) {
         message = "Variable '"+varName+"' not in Argo data file.";
         return (String) null;
      }
   ...
   /**
    * Reads the nth String from a variable in this netCDF file.
    * NOTE: The variable must be a char[N, str-length] array.
    * @param varName the string name of the variable to read
   ...
   /**
    * Reads the nth String from a variable in an open netCDF file
    * NOTE: The variable must be a char[N, str-length] array.
    * @param ncReader the netcdffile object
    * @param varName the string name of the variable to read
    * @param n the index of the variable to read
    ...
    *         null if not read (variable not present or a read error)
    */
   private static String readString (NetcdfFile ncReader, String varName, int n,
                                     boolean... returnNulls)
   {
      Variable ncVar = ncReader.findVariable(varName);
      if (ncVar == (Variable) null) {
    ...
    *         null if not read (variable not present or a read error)
    */
   public String readString (String varName, int n, int m,
                             boolean... returnNulls)
   {
    ...
    *         null if not read (variable not present or a read error)
    */
   public String readString (String varName, int n, int m, int k,
                             boolean... returnNulls)
   {
   ...
   /**
    * Reads the String array for a variable in this netCDF file.
    * NOTE: The variable must be a char[N, str-length] array.
    * @param varName the string name of the variable to read
    * @param returnNulls (OPTIONAL) true = return NULLS in string;
    *                               false (default) = truncate string if NULL is detected.
    * @return array of String representing varName[:]
    *         null if not read (variable not present or a read error)
    */
   ...
   /**
    * Reads the String array from a variable in an open netCDF file
    * NOTE: The variable must be a char[N, str-length] array.
    * Returns the String array varName[:]
    * @param ncReader the netcdffile object
    * @param varName the string name of the variable to read
    * @param returnNulls (OPTIONAL) true = return NULLS in string;
    *                               false (default) = truncate string if NULL is detected.
    ...
    *         null if not read (variable not present or a read error)
    */
   private static String[] readStringArr (NetcdfFile ncReader, String varName,
                                          boolean... returnNulls)
   {
      Variable ncVar = ncReader.findVariable(varName);
      if (ncVar == (Variable) null) {
    ...
    * Reads the String array for a variable in this netCDF file.
    * NOTE: The variable must be a char[N, M, str-length] array.
    * Returns the String array slice varName[n, :]
    * @param varName the string name of the variable to read
    * @param returnNulls (OPTIONAL) true = return NULLS in string;
    *                               false (default) = truncate string if NULL is detected.
    * @return array of String representing varName[n, :]
    *         null if not read (variable not present or a read error)
    */
    ...
    *         null if not read (variable not present or a read error)
    */
   private static String[] readStringArr (NetcdfFile ncReader, String varName, int n,
                                          boolean... returnNulls)
   {
      Variable ncVar = ncReader.findVariable(varName);
      if (ncVar == (Variable) null) {
    ...
    * Reads the String array for a variable in this netCDF file.
    * NOTE: The variable must be a char[N, M, K, str-length] array.
    * Returns the String array slice varName[n, m, :]
    * @param varName the string name of the variable to read
    * @param returnNulls (OPTIONAL) true = return NULLS in string;
    *                               false (default) = truncate string if NULL is detected.
    * @return array of String representing varName[n, m, :]
    *         null if not read (variable not present or a read error)
    */
   ...
   /**
    * Reads a int variable in the Argo file
    * NOTE: The variable must be a scalar int.
    * @param varName the string name of the variable to read
    * @return the value of the variable
    *         Integer.MAX_VALUE if not read (variable not present or a read error)
    */
   ...
   /**
    * Reads the int variable from an open netCDF file
    * NOTE: The variable must be a scalar int array.
    * @param ncReader the netcdffile object
    * @param varName the string name of the variable to read
    * @return the value of the variable
    *         Integer.MAX_VALUE if not read (variable not present or a read error)
   ...
   /**
    * Reads the nth element from an int variable from this file
    * NOTE: The variable must be a 1-D int array.
    * @param varName the string name of the variable to read
    * @param n the index of the variable to read
    * @return the value of the variable.
    *         Integer.MAX_VALUE if not read (variable not present or a read error)
   ...
   /**
    * Reads the nth element from an int variable in an open netCDF file
    * NOTE: The variable must be a 1-D int array.
    * @param ncReader the netcdffile object
    * @param varName the string name of the variable to read
    * @param n the index of the variable to read
   ...
   /**
    * Reads the int array for the variable from this file
    * NOTE: The variable must be a 1-D int array.
    * @param varName the string name of the variable to read
    * @return an int array of the values of the variable
    *         Integer.MAX_VALUE if not read (variable not present or a read error)
    */
   ...
   /**
    * Reads the int array from an open netCDF file
    * NOTE: The variable must be a 1-D int array.
    * @param ncReader the netcdffile object
    * @param varName the string name of the variable to read
    * @return an int array of the values of the variable
   ...
   /**
    * Reads the int array variable from the Argo file.
    * NOTE: The variable must be a 2-D int array.   The
    * returned array is the 1-D array slice varName[n, :].
    * @param varName the string name of the variable to read
    * @return the array of values as a int[]
    *         null if not read (variable not present or a read error)
    */
   ...
   /**
    * Reads the the int array variable from an open netCDF file
    * NOTE: The variable must be a 2-D int array.   The
    * returned array is the 1-D array slice varName[n, :].
    * @param ncReader the netcdffile object
    * @param varName the string name of the variable to read
    ...
    * Reads the int array variable from the Argo file.
    * NOTE: The variable must be a 2-D int array.   The
    * returned array is the 1-D array slice varName[n, m, :].
    * @param varName the string name of the variable to read
   ...
   /**
    * Reads a double variable in the Argo file
    * NOTE: The variable must be a scalar double.
    * @param varName the string name of the variable to read
    * @return the value of the variable
    *         Double.NaN if not read (variable not present or a read error)
    */
   ...
   /**
    * Reads the double variable from an open netCDF file
    * NOTE: The variable must be a scalar double array.
    * @param ncReader the netcdffile object
    * @param varName the string name of the variable to read
    * @return the value of the variable
    *         Double.NaN if not read (variable not present or a read error)
   ...
   /**
    * Reads the nth element from an double variable in the Argo file
   ...
   /**
    * Reads the double array variable from the Argo file
    * NOTE: The variable must be a 1-D double array.
    * @param varName the string name of the variable to read
    * @return the value of the variable
    *         null if not read (variable not present or a read error)
   ...
   /**
    * Reads the the double array variable from an open netCDF file
    * NOTE: The variable must be a 1-D double array.
    * @param ncReader the netcdffile object
    * @param varName the string name of the variable to read
    * @return the value of the variable
   ...
   /**
    * Reads the double array variable from the Argo file.
    * NOTE: The variable must be a 2-D double array.   The
    * returned array is the 1-D array slice varName[n, :].
    * @param varName the string name of the variable to read
    * @return the array of values as a double[]
    *         null if not read (variable not present or a read error)
    */
   ...
   /**
    * Reads a float variable in the Argo file
    * NOTE: The variable must be a float scalar.
    * @param varName the string name of the variable to read
    * @return the value of the variable
    *         Float.NaN if not read (variable not present or a read error)
    */
   ...
   /**
    * Reads a float variable in an open netCDF file
    * NOTE: The variable must be a scalar float.
    * @param ncReader the netcdffile object
    * @param varName the string name of the variable to read
    * @return the value of the variable
    *         Float.NaN if not read (variable not present or a read error)
   ...
   /**
    * Reads the nth element from a float array variable in the Argo file
   ...
   /**
    * Reads the float array variable from the Argo file.
    * NOTE: The variable must be a 1-D float array.
    * @param varName the string name of the variable to read
    * @return the array of values as a float[]
    *         null if not read (variable not present or a read error)
    */
   ...
   /**
    * Reads the the float array variable from an open netCDF file
    * NOTE: The variable must be a 1-D float array.
    * @param ncReader the netcdffile object
    * @param varName the string name of the variable to read
    * @return the array of values as a float[]
   ...
   /**
    * Reads the float array variable from the Argo file.
    * NOTE: The variable must be a 2-D float array.   The
    * returned array is the 1-D array slice varName[n, :].
    * @param varName the string name of the variable to read
    * @return the array of values as a float[]
    *         null if not read (variable not present or a read error)
   ...
   /**
    * Reads a short variable in the Argo file
    * NOTE: The variable must be a scalar short.
    * @param varName the string name of the variable to read
    * @return the value of the variable
    *         Short.MAX_VALUE if not read (variable not present or a read error)
    */
   ...
   /**
    * Reads the short variable from an open netCDF file
    * NOTE: The variable must be a scalar short array.
    * @param ncReader the netcdffile object
    * @param varName the string name of the variable to read
    * @return the value of the variable
    *         Short.MAX_VALUE if not read (variable not present or a read error)
   ...
   /**
    * Reads the nth element from an short variable from this file
    * NOTE: The variable must be a 1-D short array.
    * @param varName the string name of the variable to read
    * @param n the index of the variable to read
    * @return the value of the variable.
    *         Short.MAX_VALUE if not read (variable not present or a read error)
   ...
   /**
    * Reads the nth element from an short variable in an open netCDF file
    * NOTE: The variable must be a 1-D short array.
    * @param ncReader the netcdffile object
    * @param varName the string name of the variable to read
    * @param n the index of the variable to read
   ...
   /**
    * Reads the short array for the variable from this file
    * NOTE: The variable must be a 1-D short array.
    * @param varName the string name of the variable to read
    * @return an short array of the values of the variable
    *         Short.MAX_VALUE if not read (variable not present or a read error)
    */
   ...
   /**
    * Reads the short array from an open netCDF file
    * NOTE: The variable must be a 1-D short array.
   ...
   /**
    * Reads the short array variable from the Argo file.
    * NOTE: The variable must be a 2-D short array.   The
    * returned array is the 1-D array slice varName[n, :].
    * @param varName the string name of the variable to read
    * @return the array of values as a short[]
    *         null if not read (variable not present or a read error)
    */
   ...
   /**
    * Reads the the short array variable from an open netCDF file
    * NOTE: The variable must be a 2-D short array.   The
    * returned array is the 1-D array slice varName[n, :].
    * @param ncReader the netcdffile object
    * @param varName the string name of the variable to read
