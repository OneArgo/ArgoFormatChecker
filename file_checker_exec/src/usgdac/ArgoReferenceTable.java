 ...
 * @version  $HeadURL: https://inversion.nrlmry.navy.mil/svn/godae/trunk/argo/bin/java/usgdac/ArgoReferenceTable.java $
...
public final class ArgoReferenceTable
{
   ...
   //....reference table 4 --- but not quite. Also includes DAC to center-code mapping......
   public static final EnumMap<DACS,String> DacCenterCodes = 
      new EnumMap<DACS,String>(DACS.class);
      ...
      throws IOException
   {
      ...
      PLATFORM_TYPExPLATFORM_MAKER = new StringTableCrossReference
         (PLATFORM_TYPE, PLATFORM_TYPE_COLUMN, PLATFORM_MAKER_COLUMN);

      log.debug("...build PLATFORM_TYPE:WMO_INST cross-reference...");

      PLATFORM_TYPExWMO_INST = new StringTableCrossReference
         (PLATFORM_TYPE, PLATFORM_TYPE_COLUMN, WMO_INST_COLUMN);
      ...
      SENSOR_MODELxSENSOR = new StringTableCrossReference
         (SENSOR_MODEL, SENSOR_MODEL_COLUMN, SENSOR_LIST_COLUMN);

      SENSOR_MODELxSENSOR_MAKER = new StringTableCrossReference
         (SENSOR_MODEL, SENSOR_MODEL_COLUMN, SENSOR_MAKER_COLUMN);
      ...
      readSetString (specDir.trim()+ File.separator + "ref_table-29.manufacturer",
                     BATTERY_TYPE_manufacturer);

      BATTERY_TYPE_type = new LinkedHashSet<String>(10);
      readSetString (specDir.trim()+ File.separator + "ref_table-29.type",
                     BATTERY_TYPE_type);

      //..battery_packs style, type
      BATTERY_PACKS_style = new LinkedHashSet<String>(10);
      readSetString (specDir.trim()+ File.separator + "ref_table-30.style",
                     BATTERY_PACKS_style);

      BATTERY_PACKS_type = new LinkedHashSet<String>(10);
      readSetString (specDir.trim()+ File.separator + "ref_table-30.type",
                     BATTERY_PACKS_type);


      //.....measurement codes - specific codes....
      //.....measurement codes - map to JULD variables
      MEASUREMENT_CODE_specific = new IntegerTable(specDir.trim()+ File.separator +
                                                   "measurement_code-specific_codes");
      MEASUREMENT_CODE_toJuldVariable = new IntegerTable(specDir.trim()+ File.separator +
                                                       "measurement_code-juld_variables");
      ...
      readSetString (specDir.trim()+ File.separator + "generic_template_param",
                     GENERIC_TEMPLATE_param);
   }
   ...
   private void readMapCharString (String fileName, 
                                   LinkedHashMap<Character, ArgoReferenceEntry> map,
                                   LinkedHashMap<Pattern, ArgoReferenceEntry> map_re) 
      throws IOException
   {
   ...
   private void readMapStringString (String fileName, 
                                     LinkedHashMap<String, ArgoReferenceEntry>  map,
                                     LinkedHashMap<Pattern, ArgoReferenceEntry> map_re,
                                     boolean allowAppendedDigit)
      throws IOException
   {
      ...
      while ((line = fileReader.readLine()) != null) {
         ...
         if (key.indexOf('<') < 0) {
            ...
            reg = key.replaceAll("<[nN]+>", anyNumReg)
                     .replaceAll("<\\*>", anyOrNoStringReg)
   ...
   private void readMapIntegerString (String fileName, 
                                      LinkedHashMap<Integer, ArgoReferenceEntry> map,
                                      LinkedHashMap<Pattern, ArgoReferenceEntry> map_re)
      throws IOException
   {
   ...
   private void readSetString (String fileName, 
         ...
         if (! table_re.isEmpty()) {
            for (Pattern p : table_re.keySet()) {
               if (p.matcher(ch).matches()) {
                  log.debug("containsChar: table '{}'. '{}' matches regex '{}'", 
                            name, ch, p);
                  return table_re.get(p);
               }
               //log.debug("table '{}'. '{}' does NOT match '{}'", tableName, ch, p);
            }
         }
         */
   ...
   public final class IntegerTable
   {
      ...
      public ArgoReferenceEntry contains (Integer N)
      {
         ...
         /*
         //..not literal match..check for regex match
         if (! table.table_re.isEmpty()) {
            for (Pattern p : table.table_re.keySet()) {
               if (p.matcher(N).matches()) {
                  log.debug("containsInteger: table '{}'. '{}' matches regex '{}'",
                            table.name, N, p);
                  return table.table_re.get(p);
               }
   ...
   public final class StringTable
   {
      ...
      public ArgoReferenceEntry contains (String key)
      {
         ...
         if (entry != null) {
            log.debug("contains (StringTable): table '{}'. '{}' matches literal", 
                      name, str);
            return entry;
         }
         ...
         if (! table_re.isEmpty()) {
            for (Pattern p : table_re.keySet()) {
               if (p.matcher(str).matches()) {
                  log.debug("contains (StringTable): table '{}'. '{}' matches regex '{}'",
                            name, str, p);
                  return table_re.get(p);
               }
               //log.debug("table '{}'. '{}' does NOT match '{}'", tableName, name, p);
         ...
         for (String s : table.keySet()) {
            ...
            for (int n = 0; n < list.length; n++) {
               ...
               if (ss.length() != 0) {
                  ...
                  if (added) log.debug("...table entry: key, value '{}', '{}'", s, ssn);
               }
            }
         }
         ...
         for (Pattern s : table_re.keySet()) {
            ...
            for (int n = 0; n < list.length; n++) {
               ...
               if (ss.length() != 0) {
                  ...
                  if (added) log.debug("...table_re entry: key, value '{}', '{}'", s, ssn);
               }
            }
         }
