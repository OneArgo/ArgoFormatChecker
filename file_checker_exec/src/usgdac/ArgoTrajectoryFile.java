 ...
 * <p>
 * @version  $HeadURL: https://inversion.nrlmry.navy.mil/svn/godae/trunk/argo/bin/java/usgdac/ArgoTrajectoryFile.java $
...
public class ArgoTrajectoryFile extends ArgoDataFile
{
   ...
   public NetcdfFileWriter getNetcdfFileWriter () { return ncWriter; }
   ...
   {
      if (ncWriter != null) ncWriter.close();
      super.close();
   } //..end close()
    ...
    *                               in the CDL attributes to number are encountered.
    */
   public static ArgoTrajectoryFile createNew (String fileName, String specDir, 
                                               String version,
                                               int N_PARAM, int N_CYCLE, int N_HISTORY,
      ...
      throws IOException, NumberFormatException
   {
      ...
      arFile.spec = ArgoDataFile.openSpecification
         (false, specDir, ArgoDataFile.FileType.TRAJECTORY, version);
      if (arFile.spec == null) {
         return null;
      }
      ...
      arFile.ncWriter = NetcdfFileWriter.createNew(NetcdfFileWriter.Version.netcdf3,
                                                 fileName);
      ...
      if (N_PARAM <= 0)   N_PARAM = 1;
      if (N_CYCLE <= 0)   N_CYCLE = 1;
      if (N_HISTORY <= 0) N_HISTORY = 1;
                  ...
                  if (value.matches(ArgoFileSpecification.ATTR_SPECIAL_REGEX)) {
                     ...
                     if (value.length() > ArgoFileSpecification.ATTR_SPECIAL_LENGTH) {
                        ...
                        String val = value.substring(
                                      ArgoFileSpecification.ATTR_SPECIAL_LENGTH);
    ...
    *         successfully. 
    *         (ArgoTrajectoryFile.getMessage() will return the reason for the 
    *         failure to open.)
    * @throws IOException If an I/O error occurs
    */
   public static ArgoTrajectoryFile open (String inFile)
   throws IOException
   {
    ...
    * @throws IOException If an I/O error occurs
    */
   public static ArgoTrajectoryFile open (String inFile, String specDir, 
                                          boolean fullSpec)
   throws IOException
   {
   ...
   /**
    * Validates the data in the trajectory file.
    * This is a driver routine that performs all 
    * types of validations (see other validate* routines).  
    *
    * <p>NOTE: This routine (and all other validate* routines) are designed
    * to handle both core-profile and bio-profile files.  There are 
    * differnces in the validation checks that are noted where they occur.
    * 
    ...
    * @throws IOException If an I/O error occurs
    */
   public boolean validate (String dacName, 
                            boolean ckNulls)
      throws IOException
   {
      ...
      //..these are too important to all the other checks.  If fail, checking is halted

      HashMap<Integer, Integer> CycNumIndex_cycle2index = 
            new HashMap<Integer, Integer>(200);

      char[] mode_nMeasure = new char[nMeasure];

      pass = validateCycleNumber(nMeasure, nCycle, overallDM, mode_nCycle,
                                    CycNumIndex_cycle2index,
                                    mode_nMeasure);

      if (! pass) {
         //..the mapping from cyc_num to cyc_num_index is too important to continue w/o
         //will be..  formatErrors.add("CYCLE_NUMBER errors exist. Remaining checks skipped");
      ...
      Final_NMeasurement_Variables[] finalNMVars = 
            new Final_NMeasurement_Variables[nMeasure];
   ...
   /**
    * Validates the CYCLE_NUMBER variables in the trajectory file and
    * returns two variables for later processing:
    * <br> 1) a map of cycle-to-index for the [N_CYCLE] variables
    * <br> 2) the data_mode of the [N_MEASUREMENT] variables
    *
    * Bio-trajectory exceptions:
    *    does not conatin CYCLE_NUMBER_ADJUSTED
    *    does not conatin CYCLE_NUMBER_INDEX_ADJUSTED
    * 
    * Assumes:
    * <ul>
    ...
    *     <li> R  otherwise
    *     </ul>
    * <li>A map of cycle number to data index has been built for the
    *     "final CYCLE_NUMBER_INDEX" (*_ADJUSTED if set; itself if not)
    * </ul>
    *
    * Variable checks:
    ...
    * @param CycNumIndex_cycle2index  (OUTPUT!) Map of cycle_number to data index for
    *                           CYCLE_NUMBER_INDEX
    * @param mode_nMeasure char array of the data_mode values mapped onto [N_MEASUREMENT]
    * @throws IOException If an I/O error occurs
    */

   public boolean validateCycleNumber
      (int nMeasure, int nCycle, char overallDM, char[] data_mode,
       HashMap<Integer, Integer>CycNumIndex_cycle2index,
       char[] mode_nMeasure)
      throws IOException
   {
      ...
      for (int n = 0; n < nCycle; n++) {
         ...
         if (core) numAdj = cycleIndexAdj[n];
      ...
      log.debug("cycle_number_index: adjSet, dupIndex, invAdjCyc, invCyc = {}, {}, {}, {}",
                adjSet.counter, dupIndex.counter, invAdjCyc.counter, invCyc.counter);
      log.debug("cycle_number_index: missDCyc, missRCyc = {}, {}", 
                missDCyc.counter, missRCyc);

      if (adjSet.counter > 0) {
         pass = false;
         adjSet.addMessage(formatWarnings,  //will be.. formatErrors,
                           "CYCLE_NUMBER_INDEX_ADJUSTED: Set in real-time at",
                           "cycles");
      }
      if (dupIndex.counter > 0) {
         pass = false;
         dupIndex.addMessage(formatWarnings,  //will be.. formatErrors,
                             "CYCLE_NUMBER_INDEX / CYCLE_NUMBER_ADJUSTED_INDEX:"+
                             " Duplicate cycle number at ",
                             "cycles");
      }
      if (invAdjCyc.counter > 0) {
         pass = false;
         invAdjCyc.addMessage(formatWarnings,  //will be.. formatErrors,
                              "CYCLE_NUMBER_ADJUSTED_INDEX: Invalid cycle number at",
                              "cycles");
      }
      if (invCyc.counter > 0) {
         pass = false;
         invCyc.addMessage(formatWarnings,  //will be.. formatErrors,
                           "CYCLE_NUMBER_INDEX: Invalid cycle number at",
                           "cycles");
      }
      if (missDCyc.counter > 0) {
         pass = false;
         missDCyc.addMessage(formatWarnings,  //will be.. formatErrors,
                             "CYCLE_NUMBER_ADJUSTED_INDEX: Missing in delayed-mode at",
                             "cycles");
      }
      if (missRCyc.counter > 0) {
         ...
         missRCyc.addMessage(formatWarnings,  //will be.. formatErrors,
                             "CYCLE_NUMBER_INDEX: Missing in real-time at",
                             "cycles");
      }
      ...
      log.debug("cycle_number_index: adjSet, invAdjCyc, invCyc = {}, {}, {}",
                adjSet.counter, dupIndex.counter, invAdjCyc.counter, invCyc.counter);
      log.debug("cycle_number_index: invLaunch, missDCyc, missRCyc = {}, {}, {}", 
                invLaunch.counter, missDCyc.counter, missRCyc.counter);

      if (adjSet.counter > 0) {
         pass = false;
         adjSet.addMessage(formatWarnings,  //will be.. formatErrors,
                           "CYCLE_NUMBER_ADJUSTED: Set in real-time at",
                           "measurements");
      }
      if (invAdjCyc.counter > 0) {
         pass = false;
         invAdjCyc.addMessage(formatWarnings,  //will be.. formatErrors,
                              "CYCLE_NUMBER_ADJUSTED: Invalid cycle number at",
                              "measurments");
      }
      if (invCyc.counter > 0) {
         pass = false;
         invCyc.addMessage(formatWarnings,  //will be.. formatErrors,
                           "CYCLE_NUMBER: Invalid cycle number at",
                           "measurements");
      }
      if (invAdjLaunch.counter > 0) {
         pass = false;
         invAdjLaunch.addMessage(formatWarnings,  //will be.. formatErrors,
                                 "CYCLE_NUMBER_ADJUSTED: Cycle -1 not in first index at",
                                 "measurements");
      }
      if (invLaunch.counter > 0) {
         pass = false;
         invLaunch.addMessage(formatWarnings,  //will be.. formatErrors,
                              "CYCLE_NUMBER: Cycle -1 not in first index at",
                              "measurements");
      }
      if (missDCyc.counter > 0) {
         pass = false;
         missDCyc.addMessage(formatWarnings,  //will be.. formatErrors,
                             "CYCLE_NUMBER_ADJUSTED: Missing in delayed-mode at",
                             "measurements");
      }
      if (missRCyc.counter > 0) {
         pass = false;
         missRCyc.addMessage(formatWarnings,  //will be.. formatErrors,
                             "CYCLE_NUMBER: Missing in real-time at ",
                             "cycles");
      }
      ...
      int finalNum;
      int prevNum = -999;  char prevMode = 'X';
      ...
      if (missCyc.counter > 0) {
         ...
         missCyc.addMessage(formatWarnings,  //will be.. formatErrors,
                            "Cycle number in CYCLE_NUMBER/*_ADJUSTED is not "+
                            "in CYCLE_NUMBER_INDEX/*_ADJUSTED: At ",
                            " measurements");
      }
      ...
      if (missCyc.counter > 0) {
         pass = false;
         missCyc.addMessage(formatWarnings,  //will be.. formatErrors,
                            "Cycle number in CYCLE_NUMBER_INDEX/*_ADJUSTED is not "+
                            "in CYCLE_NUMBER/*_ADJUSTED: At ", "cycles");
      }
    ...
    * <br> 1) the overall data mode
    * <br> 2) the array of DATA_MODE[N_CYCLE]
    *
    * <p>Upon completion <i>obj</i>.nFormatErrors(), <i>obj</i>.nFormatWarnings, 
    * <i>obj</i>.formatErrors(), and <i>obj</i>.formatWarnings will return results.
    *
    * @param mode (output!) DATA_MODE[N_CYCLE] values
    * @return the overall data mode of the file ('D' if any D-mode cycle; 'R' otherwise)
    * @throws IOException If an I/O error occurs
    */
   public char validateDataMode(char[] mode) 
      throws IOException
   {
      ...
      invalid.addMessage(formatErrors, "DATA_MODE: Invalid at ",
                         "cycles");
    ...
    * @throws IOException If an I/O error occurs
    */
   public void validateDates (int nParam, int nHistory)
      throws IOException
   {
      ...
      if (! ref.matches(spec.getMeta(name))) {
         formatErrors.add(name+": '"+ref+
               "': Does not match specification ('"+spec.getMeta(name)+"')");
      }
         ...
         if (dateCreation == (Date) null) {
            ...
            if (dateCreation.before(earliestDate)) {
               formatErrors.add("DATE_CREATION: '"+creation+
                  "': Before allowed date ('"+ArgoDate.format(earliestDate)+"')");
    ...
    *
    * Bio-trajectory exception:
    *   does not contain CYCLE_NUMBER_ADJUSTED
    *   does not contain JULD_ADJUSTED*(N_MEASUREMENT) variables
    *   does not contain the JULD_*(N_CYCLE) variables
    * 
    * Assumes:
    * <ul>
    * <li>DATA_MODE has been read and validated (and passed into routine)
    * <li>CYCLE_NUMBER variables (N_MEASUREMENT and N_CYCLES) have been
      ...
      throws IOException
   {
      ...
      for (int n = 0; n < nMeasure; n++) {
         ...
         if (m_code[n] == 0) {
            ...
            //..invalid codes
            invCode.increment(n);
            log.debug("m_code[{}]: invalid code = {}, rem = {}, rel = {}",
                      n, m_code[n], rem, rel);
            ...
            if (info.isValid()) {
               if (info.isDeprecated) {
                  depCode.increment(n);
                  log.debug("m_code[{}]: deprecated code = {}, rem = {}, rel = {}",
                            n, m_code[n], rem, rel);
               }

            } else if (info.isDeleted) {
               delCode.increment(n);
               log.debug("m_code[{}]: deleted code = {}, rem = {}, rel = {}",
                         n, m_code[n], rem, rel);
      ...
      invCode.addMessage(formatWarnings,   //will be..  formatErrors,
                         "MEASUREMENT_CODE: Invalid measurement codes at ",
                         "measurements");
      depCode.addMessage(formatWarnings,
                         "MEASUREMENT_CODE: Deprecated measurement codes at ",
                         "measurements");
      delCode.addMessage(formatWarnings,   //will be..  formatErrors,
                         "MEASUREMENT_CODE: Obsolete measurement codes at ",
                         "measurements");
      ...
      for (int n = 0; n < nMeasure; n++) {
         ...
         if ( (juld_qc[n] == ' ')  ^  (juld_status[n] == ' ') ) {
               ...
               log.debug("qc/status inconsistent: "+
                         "n = {}: juld, juld_status, juld_qc = {}, '{}', '{}'",
                         n, juld[n], juld_status[n], juld_qc[n]);
         ...
         if (juld_qc[n] == ' ' || juld_qc[n] == '9') {
            if (! (juld_status[n] == ' ' || juld_status[n] == '9') ) {
               incStatus.increment(n);
               log.debug("qc/status inconsistent: "+
                         "n = {}: juld, juld_status, juld_qc = {}, '{}', '{}'",
                         n, juld[n], juld_status[n], juld_qc[n]);
      ...
      log.debug("invQC, invStatus, incStatus, notMiss, noQC = {}, {}, {}, {}, {}",
                invQC.counter, invStatus.counter, incStatus.counter,
                notMiss.counter, noQC.counter);
      /*log.debug("invQC, invStatus, inc_Status, inc9Status, notMiss, noQC = {}, {}, {}, {}, {}, {}",
      ...
      */
      ...
      if (invQC.counter > 0) {
         ...
         invQC.addMessage(formatWarnings,   //will be..  formatErrors,
                          "JULD_QC: Invalid QC code at",
                          "measurements");
      }
      if (depQC.counter > 0) {
         //fail = true;
         depQC.addMessage(formatWarnings,
                          "JULD_QC: Deprecated QC code at",
                          "measurements");
      }
      if (invStatus.counter > 0) {
         //fail = true;
         invStatus.addMessage(formatWarnings,   //will be..  formatErrors,
                              "JULD_STATUS: Invalid status code at",
                              "measurements");
      }
      if (depStatus.counter > 0) {
         //fail = true;
         depStatus.addMessage(formatWarnings,
                              "JULD_STATUS: Deprecated status code at",
                              "measurements");
      }
      if (incStatus.counter > 0) {
         //fail = true;
         incStatus.addMessage
            (formatWarnings,   //will be..  formatErrors,
             "JULD_QC / JULD_STATUS: Use of ' ' or '9' is inconsistent at",
             "measurements");
      }
      /*
      if (inc_Status.counter > 0) {
         //fail = true;
         inc_Status.addMessage(formatErrors,
                              "JULD_QC = ' '; JULD_STATUS not ' ': At",
                              "measurements");
      }
      if (inc9Status.counter > 0) {
         //fail = true;
         inc_Status.addMessage(formatErrors,
                              "JULD_QC = '9'; JULD_STATUS not '9': At",
                              "measurements");
      }
      */
      if (notMiss.counter > 0) {
         ...
         notMiss.addMessage(formatWarnings,   //will be..  formatErrors,
                            "JULD: FillValue where QC not ' ' or '9': At",
                            "measurements");
      }
      if (noQC.counter > 0) {
         //fail = true;
         noQC.addMessage(formatWarnings,   //will be..  formatErrors,
                         "JULD: Not FillValue where QC ' ' or '9': At",
                         "measurements");
      }
      ...
      if (core) {
         ...
         for (int n = 0; n < nMeasure; n++) {
            ...
            if ( (juld_adj_qc[n] == ' ')  ^  (juld_adj_status[n] == ' ') ) {
               ...
               log.debug("qc/status inconsistent: "+
                         "n = {}: juld, juld_status, juld_qc = {}, '{}', '{}'",
                         n, juld[n], juld_adj_status[n], juld_adj_qc[n]);
            ...
            /*
              if (juld_adj_qc[n] == ' ') {
              if (juld_adj_status[n] != ' ') {
              inc_Status.increment(n);
              log.debug("qc/status inconsistent: "+
              "n = {}: juld_adj, juld_adj_status, juld_adj_qc = {}, '{}', '{}'",
              n, juld_adj[n], juld_adj_status[n], juld_adj_qc[n]);
              }
              }
              if (juld_adj_qc[n] == '9') {
              if (juld_adj_status[n] != '9') {
              inc9Status.increment(n);
              log.debug("qc/status inconsistent: "+
              "n = {}: juld_adj, juld_adj_status, juld_adj_qc = {}, '{}', '{}'",
              n, juld_adj[n], juld_adj_status[n], juld_adj_qc[n]);
              }
            ...
            */
            /*
              ...
              if (! (juld_adj_status[n] == ' ' || juld_adj_status[n] == '9') ) {
              incStatus.increment(n);
              log.debug("qc/status inconsistent: "+
              "n = {}: juld_adj, juld_adj_status, juld_adj_qc = {}, '{}', '{}'",
              n, juld_adj[n], juld_adj_status[n], juld_adj_qc[n]);
         ...
         log.debug("invQC, invStatus, incStatus = {}, {}, {}",
                   invQC.counter, invStatus.counter, incStatus.counter);
         //log.debug("invQC, invStatus, inc_Status, inc9Status = {}, {}, {}, {}",
         //          invQC.counter, invStatus.counter, inc_Status.counter, inc9Status.counter);
         log.debug("notMiss, noQC, adjNotAorD = {}, {}, {}",
                   notMiss.counter, noQC.counter, adjNotAorD.counter);

         if (invQC.counter > 0) {
            //fail = true;
            invQC.addMessage(formatWarnings,   //will be..  formatErrors,
                             "JULD_ADJUSTED_QC: Invalid QC code at",
                             "measurements");
         }
         if (depQC.counter > 0) {
            //fail = true;
            depQC.addMessage(formatWarnings,
                             "JULD_ADJUSTED_QC: Deprecated QC code at",
                             "measurements");
         }
         if (invStatus.counter > 0) {
            //fail = true;
            invStatus.addMessage(formatWarnings,   //will be..  formatErrors,
                                 "JULD_ADJUSTED_STATUS: Invalid status code at",
                                 "measurements");
         }
         if (depStatus.counter > 0) {
            //fail = true;
            depStatus.addMessage(formatWarnings,
                                 "JULD_ADJUSTED_STATUS: Deprecated status code at",
                                 "measurements");
         }
         ...
         if (incStatus.counter > 0) {
            ...
            incStatus.addMessage
               (formatWarnings,   //will be..  formatErrors,
                "JULD_ADJUSTED_QC / JULD_ADJUSTED_STATUS: Use of ' '/'9' is inconsistent at",
                "measurements");
         }
         ...
         /*
           ...
           //fail = true;
           inc_Status.addMessage(formatErrors,
           "JULD_ADJUSTED_QC = ' '; JULD_ADJUSTED_STATUS not ' ': At",
           "measurements");
           }
           if (inc9Status.counter > 0) {
           //fail = true;
           inc9Status.addMessage(formatErrors,
           "JULD_ADJUSTED_QC = '9'; JULD_ADJUSTED_STATUS not '9': At",
           "measurements");
           }
         */
         /*
         ...
         */
         if (notMiss.counter > 0) {
            //fail = true;
            notMiss.addMessage(formatWarnings,   //will be..  formatErrors,
                               "JULD_ADJUSTED: FillValue where QC not ' ' or '9': At",
                               "measurements");
         }
         if (noQC.counter > 0) {
            //fail = true;
            noQC.addMessage(formatWarnings,   //will be..  formatErrors,
                            "JULD_ADJUSTED: Not FillValue where QC ' ' or '9': At",
                            "measurements");
         }
         if (adjNotAorD.counter > 0) {
            //fail = true;
            adjNotAorD.addMessage(formatWarnings,   //will be..  formatErrors,
                                  "JULD_ADJUSTED: Not FillValue where DATA_MODE not 'A' or 'D': At",
                                  " measurements");
         }
    ...
    * 
    * Meta-data checks
    *   PLATFORM_NUMBER: Valid (5 or 7 numeric digits)
    *   DATA_CENTRE: Valid value - Ref table 4
    *   DATA_STATE_INDICATOR: Valid value - Ref table 6
    *   FIRMWARE_VERSION: Not empty
    *   FLOAT_SERIAL_NO: Not empty
    *   PLATFORM_TYPE: Valid value - Ref table 23
    *   POSITIONING_SYSTEM: Valid value - Ref table 9
    ...
    *
    * @param nProf the number of profiles in the file
    * @param dac the ArgoReferenceTable.DACS dac indicator.  
    *            If <i>null</i> the DATA_CENTRE cannont be
    * validated.
    * @throws IOException If an I/O error occurs
    */
   public boolean validateMetaData (ArgoReferenceTable.DACS dac)
   throws IOException
   {
   ...
   public void validateNCycle(int nCycle, char[] mode)
   {
      ...
      if (fileType == FileType.TRAJECTORY) {
         ...
         //will be.. inv.addMessage(formatErrors, varName+": Invalid code at ", "cycles");
      ...
      //will be..  inv.addMessage(formatErrors, varName+": Invalid value at", "cycles");
    ...
    * JULD(N_MEASUREMENT) settings to the corresponding JULD_* variable based
    * on the MEASUREMENT_CODE
    *
    * Bio-trajectory exception:
    *    does not contain *any* JULD_*(N_CYCLE) variables
    *
    * Assumes:
    * <ul>
   ...
   public void validateNCycleJuld
      (int nMeasure, int nCycle, //char[] mode,
       HashMap<Integer, Integer>CycNumIndex_cycle2index,
       Final_NMeasurement_Variables[] finalNMVar)
   {
      ...
      if (finalNMVar[0].cycle_number == -1) startNMeasureLoop = 1;
      ...
      for (Integer MC : ArgoReferenceTable.MEASUREMENT_CODE_toJuldVariable.keySet()) {
         ...
         String var = 
            ArgoReferenceTable.MEASUREMENT_CODE_toJuldVariable.get(MC).getColumn(2);
         ...
         if (M_CODE > 10000) {
            ...
            log.debug("flagged MC = {}:  first, last = {} {}",
                      M_CODE, only_first, only_last);
         }
         ...
         for (int n = startNMeasureLoop; n < nMeasure; n++) {
            ...
            if (cycNum != prev_cycNum) {
               if (do_onlylast) {
                  ...
                  juldCheck.check
                     (last_n, finalNMVar[last_n].juld, finalNMVar[last_n].juld_status,
                      last_index, juldVar[last_index], juldVar_status[last_index]);

                  do_onlylast = false;

                  log.debug("(temp) only_last: mc {}: juld[{}] = {} {} : {}[{}] = {} {}",
                  mc, last_n, finalNMVar[last_n].juld, finalNMVar[last_n].juld_status,
                  var, last_index, juldVar[last_index], juldVar_status[last_index]);
               }
            ...
            if (mc == M_CODE) {
               ...
               if (ndx == null) {
                  ...
                  log.error("validateNCycleJuld: CycNumIndex_cycle2index[{}] is null. "+
                            "This should not happen", cycNum);
                  continue;
               }
               ...
               juldCheck.check(n, finalNMVar[n].juld, finalNMVar[n].juld_status,
                               index, juldVar[index], juldVar_status[index]);

               log.debug("(temp) mc {}: juld[{}] = {} {} : {}[{}] = {} {}",
                         mc, n, finalNMVar[n].juld, finalNMVar[n].juld_status,
                         var, index, juldVar[index], juldVar_status[index]);
         ...
         if (do_onlylast) {
            ...
            log.debug("(temp) dangling only_last: mc {}: juld[{}] = {} {} : {}[{}] = {} {}",
                  mc, last_n, finalNMVar[last_n].juld, finalNMVar[last_n].juld_status,
                  var, last_index, juldVar[last_index], juldVar_status[last_index]);
         }
         ...
         if (juldCheck.incJuld.counter > 0) {
            juldCheck.incJuld.addMessage(formatWarnings,   //will be..  formatErrors,
                "JULD (MC "+M_CODE+") / "+var+ ": Inconsistent at ",
                "cycles", "(N_MEASUREMENT, N_CYCLE)");
         ...
         if (notJuld > 0) {
            ...
            formatWarnings.add(var+" (MC "+M_CODE+
                "): Not FillValue where there is no associated JULD at "+
                notJuld+" cycles; index of first case = "+a_notJuld);
         }
         if (notJuld_s > 0) {
            formatWarnings.add(var+"_STATUS (MC "+M_CODE+
                "): Not FillValue where there is no associated JULD_STATUS at "+
                             notJuld_s+" cycles; index of first case = "+
                             a_notJuld_s);
         }
   ...
   public void validateParams
      (int nMeasure, char[] mode_nMeasure,
      ...
      throws IOException
   {
      ...
      for (int nPrm =0 ; nPrm < paramList.size(); nPrm++) {
            ...
            if (prm_qc != null) {
               ...
               if (prm_qc[n] == ' ' || info.isValid()) {
                  ...
                  if (ArgoDataFile.is_FillValue(fValue, prm[n])) {
                     //..data is missing - QC better be too
                     if (prm_qc[n] != '9' &&  prm_qc[n] != ' ') { notMiss.increment(n); }
         ...
         log.debug("...counters: invQC, missQC, notMiss, nan = {}, {}, {}, {}",
                     invQC.counter, missQC.counter, notMiss.counter, nan.counter);
         ...
         invQC.addMessage(formatWarnings,
                          varName+"_QC: Deprecated QC code at", "measurements");

         if (missQC.counter > 0) {
            fail = true;
            missQC.addMessage(formatWarnings,   //will be..  formatErrors, 
                              varName+": Not FillValue where QC is ' ' or 9 at",
                              "measurements");
         }
         if (notMiss.counter > 0) {
            fail = true;
            notMiss.addMessage(formatWarnings,   //will be..  formatErrors, 
                               varName+": FillValue where QC is not ' ' or 9 at",
                               "measurements");
         }
         if (nan.counter > 0) {
            fail = true;
                     ...
                     if (ArgoDataFile.is_FillValue(fValue, prm_adj[n])) {
                        ...
                        if (prm_adj_qc[n] == '4' || prm_adj_qc[n] == '9') {
                           if (mode[n] == 'D') {
                              ...
                              if (prm_adj_qc[n] == '9') adjNotMiss.increment(n);
                           }
         ...
         if (log.isDebugEnabled()) {
            ...
            log.debug("...counters: incNotMeas, invQC, depQC, missAdj = {}, {}, {}, {}",
                  incNotMeas.counter, invQC.counter, depQC.counter, missAdj.counter);
            log.debug("...counters: nan, nanErr = {}, {}",
                  nan.counter, nanErr.counter);
            ...
            log.debug("...counters: rErrNotMiss, rNotMiss, rQcNotMiss = {}, {}, {}",
                  rErrNotMiss.counter, rNotMiss.counter, rQcNotMiss.counter);
         }
   ...
   private float[] collapse_extra (Array array, int nMeasure, Variable var, float fillValue)
      throws IOException
   {
      ...
      for (int n = 0; n < nMeasure; n++) {
         ...
         if (is_nan) { prm[n] = Float.NaN; } //..NaN trumps everything
         else if (data != Float.MAX_VALUE) { prm[n] = data; } //..had data
         else { prm[n] = fillValue; }
      }
   ...
   /**
    * Validates the N_MEASUREMENT "position variables".
    * This is the N_MEASURMENT variables except:
    *    CYCLE_NUMBER, MEASURMENT_CODE, JULD
    * 
    * Assumes:
    * <ul>
    * <li>DATA_MODE has been read and validated (and passed into routine)
   ...
   public void validatePosition (int nMeasure)
      throws IOException
   {
      ...
      //will be.. invCode.addMessage(formatErrors, "POSITION_QC: Invalid QC code at ", "measurements");
      invCode.addMessage(formatWarnings, "POSITION_QC: Invalid QC code at ", "measurements");

      depCode.addMessage(formatWarnings, "POSITION_QC: Deprecated QC code at ",
                         "measurements");
      ...
      depCode.addMessage(formatWarnings,
                         "POSITION_ACCURACY: Deprecated code at ", " measurements");
      ...
      notMissQC.addMessage(formatWarnings,   //will be..  formatErrors,
                           "LAT/LON missing: QC is not 9 or ' ' at ",
                           "measurements");

      notMissPos.addMessage(formatWarnings,   //will be..  formatErrors,
                            "POSITION_QC = 9 or ' ': LAT/LON not missing at ",
                            " measurements");
    ...
    * @throws IOException If an I/O error occurs
    */
   public ArrayList<String> validateTrajectoryParameters(int nParam)
      throws IOException
   {
         ...
         if (param.length() == 0) {
            ...
            if (allowedParam.contains(param)) {
               ...
               if (paramList.contains(param)) {
                  ...
                  formatErrors.add("TRAJECTORY_PARAMETERS["+(paramNum+1)+"]: '"+param+
                                   "': Duplicate entry");
   ...
   /**
    * Inner class to support error tracking
    * This class has convenience functions to maintain lists of indices for errors,
    *
    */
   private class ErrorTracker
   {
      ...
      /**
       * Add index to array of error-indexes
       * (if it is less the maximum number that can be tracked)
       */
      protected void increment(int index, String... label) {
         if (counter < N_TRACKED) {
            indices1[counter] = index+1;
            if (label.length > 0) log.debug("{}: index = {}", label[0], index);
         }
      ...
      /**
       * Add index range to array of error-indexes
       * (if it is less the maximum number that can be tracked)
       */
      protected void increment(int index1, int index2, String... label) {
         if (counter < N_TRACKED) {
            ...
            if (label.length > 0) log.debug("{}: index1, 2 = {}, {}", 
                                            label[0], index1, index2);
         }
   ...
   private class ValidateNCycleJuld_check
   {
      ...
      public void check(int ndx, double juld, char juld_status, 
                        int ndxIndex, double juldIndex, char juldIndex_status) {
